...
//!
//! This command validates only the framing and checksum of the 712-byte outer
//! envelope. The rendered metadata and roots remain self-asserted: nested
//! composition is not verified, no closure or settlement claim is promoted,
//! and no authority is granted. Payloads, prompts, responses, and credentials
//! are never rendered.
...
fn run() !void {
    ...
    try renderEnvelopeV1(&output.interface, inspected);
    try output.interface.flush();
}

fn parseOptions(arguments: []const []const u8) !Options {
    if (arguments.len != 3 or
        !std.mem.eql(u8, arguments[1], "--join") or
        arguments[2].len == 0)
        return error.InvalidArguments;
    return .{ .join_path = arguments[2] };
}
...
fn readExactRegularFile(path: []const u8) ![join.encoded_bytes]u8 {
    ...
    if (before.size != join.encoded_bytes)
        return error.InvalidInputLength;

    var encoded: [join.encoded_bytes]u8 = undefined;
    if (try file.readAll(&encoded) != encoded.len)
        return error.InputChanged;
    var probe: [1]u8 = undefined;
    if (try file.pread(&probe, join.encoded_bytes) != 0)
        return error.InputChanged;
...
fn renderEnvelopeV1(
    ...
    inspected: join.InspectedEnvelopeV1,
) !void {
...
test "provider evidence inspector accepts only one join path" {
...
test "provider evidence inspector renders deterministic outer-only JSON" {
    const inspected: join.InspectedEnvelopeV1 = .{
        .self_asserted = testDecodedV1(),
    };
    var first_storage: [8192]u8 = undefined;
    var first_writer = std.Io.Writer.fixed(&first_storage);
    try renderEnvelopeV1(&first_writer, inspected);

    var second_storage: [8192]u8 = undefined;
    var second_writer = std.Io.Writer.fixed(&second_storage);
    try renderEnvelopeV1(&second_writer, inspected);
