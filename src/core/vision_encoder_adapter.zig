...
test "vision adapter adopts scheduled receipt and publishes on final quantum" {
    var fixture = try TestFixture.init();
    try fixture.rebind();

    var cache_slots = [_]resource_bank.Slot{.{}} ** 8;
    var cache_roots = [_]resource_bank.LeaseTreeRootSlot{.{}} ** 8;
    var cache_nodes = [_]resource_bank.LeaseNodeSlot{.{}} ** 12;
    ...
    var scheduler = try qos.Scheduler.init(
        ...
        .{
            .scheduler_epoch = 0x5602,
            .challenge = model.sha256("scheduled vision challenge"),
            .max_weight = 1,
    ...
    const descriptor = try makeAdapterDescriptorV1(
        reference_adapter_abi,
        fixture.manifest,
        model.sha256("scheduled reference vision projection v1"),
    );
    var context: u8 = 1;
    const adapter: AdapterV1 = .{
        .context = &context,
