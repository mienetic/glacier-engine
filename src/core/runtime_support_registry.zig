...
pub const profiles = [_]ProfileV1{
    ...
    .{
        .index = .dense_tensor_embedding,
        .slug = "dense-tensor-embedding-reference",
        .profile_abi = dense_tensor_embedding.reference_adapter_abi,
        .lifecycle = .stateless,
        .evidence = .retained_reference_fixture,
...
test "registry profiles are append-only views of adapter support constants" {
    ...
    try std.testing.expectEqual(
        dense_tensor_embedding.reference_adapter_abi,
        profiles[9].profile_abi,
    );
    try std.testing.expectEqual(
...
test "query returns only the normalized embedding bit at exact bounds" {
    const support = profiles[9].support;
    const result = querySupportV1(queryFor(
        support,
        support.max_batch_items,
        support.max_input_features,
    ...
    try std.testing.expectEqual(
        @as(u64, 1) << @intFromEnum(ProfileIndexV1.dense_tensor_embedding),
        result.matching_profile_mask,
    );
    try std.testing.expectEqual(
        @as(?model.UnsupportedReasonV1, null),
    ...
    const rejected = querySupportV1(queryFor(
        support,
        support.max_batch_items,
        support.max_input_features,
    ...
    try std.testing.expectEqual(@as(u64, 0), rejected.matching_profile_mask);
    try std.testing.expectEqual(
