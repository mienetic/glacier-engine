...
test "temporal video adapter adopts scheduled receipt and publishes on final quantum" {
    var fixture = try TestFixture.init();
    try fixture.rebind();
    ...
    var scheduler = try qos.Scheduler.init(
        ...
        .{
            ...
            .challenge = model.sha256("scheduled video challenge"),
            .max_weight = 1,
    ...
    const descriptor = try makeAdapterDescriptorV1(
        fixture.manifest,
        model.sha256("scheduled reference video projection v1"),
    );
    var context: u8 = 1;
    const adapter: AdapterV1 = .{
        .context = &context,
