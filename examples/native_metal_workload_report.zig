//! Emit one production-native Metal Workload Report V1 as a raw binary wire.
//!
...
const build_domain = "glacier-w6-metal-native-build-v1\x00";

pub fn main() !void {
    if (!engine.metal_enabled)
        return error.NativeMetalWorkloadReportRequiresMetal;
    ...
    const artifact = try producer.runV1(
        &storage,
        &backend,
        config,
    );
