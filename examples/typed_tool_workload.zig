//! Canonical download-free typed-tool workload report.

const std = @import("std");
const core = @import("core");
const contract = core.typed_workload_contract;
const driver = core.typed_workload_driver;
const tool = core.tool_action_contract;
const workload = core.typed_tool_workload;
...
pub fn main() !void {
    ...
    var storage = try workload.ReferenceStorageV1.init();
    const campaign = try workload.runReferenceCampaignV1(&storage);
    var replay_storage: driver.MaximumStorageV1 = .{};
    try workload.validateEvidenceByReplayV1(
        campaign.plan,
    ...
    try writer.print(
        "{{\"schema\":\"glacier.typed-tool-conformance/v1\"," ++
            "\"plan_abi\":\"{x:0>16}\"," ++
            "\"profile_abi\":\"{x:0>16}\"," ++
            "\"item_abi\":\"{x:0>16}\"," ++
            ...
            "\"tool_descriptor_abi\":\"{x:0>16}\"," ++
            "\"tool_arguments_abi\":\"{x:0>16}\"," ++
            "\"tool_proposal_abi\":\"{x:0>16}\"," ++
            "\"tool_policy_abi\":\"{x:0>16}\"," ++
            "\"tool_authorization_abi\":\"{x:0>16}\"," ++
            "\"tool_effect_abi\":\"{x:0>16}\"," ++
            "\"tool_delivery_abi\":\"{x:0>16}\"," ++
            "\"evidence_abi\":\"{x:0>16}\"," ++
        ...
        .{
            ...
            driver.summary_abi,
            tool.descriptor_abi,
            tool.bounded_add_arguments_abi,
            tool.proposal_abi,
            tool.policy_abi,
            tool.authorization_abi,
            tool.effect_abi,
            tool.delivery_abi,
            workload.evidence_abi,
            workload.item_evidence_abi,
            workload.summary_abi,
    ...
    try writer.writeAll("],\"dispositions\":[");
    for (campaign.evidence.items, 0..) |item, index| {
        if (index != 0) try writer.writeByte(',');
        if (item.delivery) |delivery| {
            try writer.print(
                "\"{s}\"",
                .{@tagName(delivery.disposition)},
            );
        } else {
...
fn writeEvidenceSummary(
    writer: anytype,
    summary: workload.SummaryV1,
) !void {
    try writer.print(
        "{{\"profile_count\":{d},\"item_count\":{d}," ++
            "\"admitted\":{d},\"rejected\":{d}," ++
            "\"completed\":{d},\"cancelled\":{d}," ++
            "\"timed_out\":{d},\"tool_calls\":{d}," ++
            "\"deliveries\":{d},\"executed\":{d}," ++
            "\"reused\":{d},\"denied\":{d},\"conflicts\":{d}," ++
            "\"effects\":{d},\"initial_counter\":{d}," ++
            "\"final_counter\":{d}," ++
            "\"model_successful_commits\":{d}," ++
            "\"model_releases\":{d}," ++
            "\"model_final_active_reservations\":{d}," ++
            "\"model_final_committed_receipts\":{d}," ++
            "\"harness_open\":{s},\"pending_prepared\":{d}," ++
            "\"pending_armed\":{d}," ++
            "\"zero_model_ownership\":{s}," ++
            "\"zero_harness_authority\":{s}," ++
            "\"zero_orphan_ownership\":{s}}}",
        .{
            summary.profile_count,
            summary.item_count,
            ...
            summary.conflicts,
            summary.effects,
            summary.initial_counter,
            summary.final_counter,
            ...
            boolJson(summary.harness_open),
            summary.pending_prepared,
            summary.pending_armed,
            boolJson(summary.zero_model_ownership),
            boolJson(summary.zero_harness_authority),
            boolJson(summary.zero_orphan_ownership),
        },
    );
}

fn writeClaim(writer: anytype, claim: core.resource_bank.Claim) !void {
    try writer.print(
        "{{\"capsule_bytes\":{d},\"kv_bytes\":{d}," ++
            "\"activation_bytes\":{d},\"partial_bytes\":{d}," ++
            "\"output_journal_bytes\":{d},\"staging_bytes\":{d}," ++
            "\"queue_slots\":{d}}}",
        .{
            claim.capsule_bytes,
            claim.kv_bytes,
