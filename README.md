...
- **Bounded unary lifecycle kernel.** The experimental
  ...
  CLI is its first compatibility-checked consumer. Network transport and
...
its compile-only companion instead of the broad model-forward suite. When the
CLI changes, both focused roots share one Zig invocation. If a change set also
needs the generic package/contract roots, those roots join the same invocation.
