...
Kernel implementation changes run the service and HTTP roots in one host Zig
invocation. When the CLI changes, its selected focused roots likewise share
one invocation. If a change set also needs the generic package/contract roots,
those roots join that invocation.
