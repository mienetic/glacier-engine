...
| Scheduling | Exact admission, deterministic weighted QoS, one bounded mixed-media pressure campaign with exact replay, and final-quantum image/audio/video media transactions under the scheduler-owned receipt | Family-aware batching, preemption, multi-device placement, typed model-adapter workloads, and broader multi-tenant campaigns |
...
| Load and resilience | Prototype: one portable explicit-open-loop scenario/result contract covers mixed-media overload, fairness, timeout, cancellation, logical resource high-water marks, and zero-orphan close; an additive sidecar executes and verifies one completed image, audio, and video media transaction without double admission | Generated scenarios, a separate closed-loop mode, typed model/provider/tool workloads, native p50/p95/p99 and physical-resource evidence, then bounded soak and disruption campaigns |
