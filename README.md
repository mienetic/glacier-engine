...
- **Typed model-family execution.** Fixed artifact, execution-plan, and result
  ...
  vision, audio, temporal-video, dense-tensor reranking, and normalized
  dense-embedding adapters compute into provisional storage, reject candidate
  drift, and publish source- and ownership-bound typed transactions. The
  generic tensor fixtures retain explicit batch identity, deterministic
  ranking or exact Q30 L2 policy, and require no model download. Video
...
- **Inspectable reference compatibility.** A deterministic read-only command
  renders ten append-only exact-integer fixture profiles as versioned JSON.
...
- **Bounded loopback HTTP/1.1.** An experimental serial socket adapter exposes
  ...
  drain cancellation plus an optional 1-millisecond-to-60-second monotonic
  receive deadline for partial headers and bodies, with separate evidence for
  all three outcomes. Non-loopback serving, authentication, TLS, streaming,
  full-request timeout, disconnect or blocked-response-write cancellation,
  durable restart, and performance evidence remain open.
...
Shared server adapter/API changes add `unary-server-process-test` and its
compile companion without selecting the service-only root. Managed
server-process acceptance changes run `unary-server-process-test`, with
`unary-server-process-compile` reserved for retained-target compile evidence.
The Phase B receive-drain, Phase C monotonic receive-timeout, and Phase D
admitted-work drain cases reuse that same dual-mode executable and those same
targets; they add no compile root. A unary-kernel implementation change selects
the service, HTTP, and managed-process roots. A shared server implementation
change selects the HTTP and managed-process roots without the service-only
root. All selected host roots share one Zig invocation. When the CLI changes,
its selected focused roots likewise share that invocation. If a change set also
needs the generic package/contract roots, those roots join it.
