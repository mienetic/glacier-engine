...
- **Device allocation ownership contracts.** A fixed-storage allocator boundary
  ...
  completion, and terminal command errors instead retain a sticky, nonterminal
  `MetalAsyncDispatchQuarantineV1`; they never manufacture core terminal
  evidence. This is one adapter-owned slot, not a global native queue-depth
  limit or multi-slot scheduler.
...
| Device runtime | Portable capability selection; native Metal fingerprint revalidation; adapter-quoted fake and real-Metal multi-buffer allocation; exact charge-before-allocate accounting; ChildLease and additive LeaseTree ownership; private reserve/materialize/FreePermit authority; bounded object-set pins; adapter-issued generation-fenced Metal request roots; sealed pre-Bank pin intents with exact post-reserve abort; submitted, pure-cancelled, and inspected-but-unsubmitted rejection branches; per-adapter single-flight Metal async completion delivery through `MetalAsyncDispatchTicketV1`; sticky nonterminal `MetalAsyncDispatchQuarantineV1`; exact native command/output binding; post-Bank native finalization; replay tombstones; one real four-buffer pinned INT4 command checked by a CPU oracle; direct Metal length/`allocatedSize` observation; generation-fenced reuse; cancellation rollback; conservative recovery; sibling isolation; and asymmetric FP16 tiled-matmul CPU-oracle tests | Device-loss inspection and reconciliation, quarantine clearing, and fresh selection; multi-slot and multi-device queue scheduling; separate physical residency and direct telemetry; additional GPU backends; retained native OS/device matrices; then performance evidence under declared campaigns |
