...
- **Fail-closed native observation.** A pointer-free W5a contract records host
  ...
  readable reasons for unavailable JSON metrics without making unavailable
  temperature, frequency, power, energy, or GPU telemetry claims.
...
| Runtime Workload Lab | W0 deterministic mixed-media open-loop conformance, W1 scheduler-coupled media execution, the W2 four-seed/32-case generated corpus, W3 finite-source closed-loop conformance, W4a mixed typed-perception conformance, the W4b-a typed tool transaction, W4b-b ActionOutbox record recovery, the W4b-c durable POSIX store, W4b-d generation-fenced fake dispatch/status, and the first W5a native-observation contract/runner cover overload, fairness, timeout, cancellation, turnover, typed publication/effect delivery, uncertain external handoff, fenced safe retry, deterministic crash modeling, explicit machine-state availability, fail-closed pre-run admission, and retained post-run contamination | Direct CPU/GPU power, temperature, frequency, energy, residency, utilization and queue adapters; native latency/throughput reports, bounded soak, disruption, and multi-OS replication |
