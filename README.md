...
- **Typed model-family execution.** Fixed artifact, execution-plan, and result
  ...
  and generic dense-tensor classification adapters compute into provisional
  storage, reject candidate drift, and publish source- and ownership-bound
  typed transactions. The generic tensor fixtures retain explicit batch
  identity plus canonical rank, normalization, or class-score policy and
  require no model download. Video selection gathers strided frames through
  explicitly charged scratch and scrubs it on return.
- **Inspectable reference compatibility.** A deterministic read-only command
  renders eleven append-only exact-integer fixture profiles as versioned JSON.
