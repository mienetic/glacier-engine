...
| Runtime Workload Lab | W0 deterministic mixed-media open-loop conformance, W1 scheduler-coupled media execution, the W2 four-seed/32-case generated corpus, W3 finite-source closed-loop conformance, W4a mixed typed-perception conformance, and the first W4b typed tool transaction cover overload, fairness, timeout, cancellation, turnover, exact typed publication/effect delivery, replay, and zero-orphan close | Provider, stateful, streaming, batched, preemptible, device-backed, and durable live-tool W4 profiles; then native latency/throughput reports with CPU/GPU placement, utilization, memory, fallback, and available power/thermal evidence, bounded soak, disruption, and multi-OS replication |
