...
- **Device allocation ownership contracts.** A fixed-storage allocator boundary
  ...
  command is live, and consumes the private pin only after the exact bound
  adapter validates terminal evidence. The native allocation gate composes
  that contract with four real registry-owned Metal buffers, one synchronous
  INT4 dispatch, and a CPU-oracle result check.
...
| Device runtime | Portable capability selection, native Metal fingerprint revalidation, adapter-quoted fake and real-Metal multi-buffer allocation paths, exact charge-before-allocate accounting (logical `MTLBuffer.length` for Metal V1), ChildLease and execution-owned additive LeaseTree coordinators, private reserve/materialize/FreePermit authority, bounded exact object-set dispatch pins, terminal-evidence completion fencing, one real four-buffer pinned Metal INT4 dispatch with a CPU oracle, direct per-resource Metal length/`allocatedSize` observation, generation-fenced object sets and slot reuse, cancellation rollback, conservative recovery, sibling-scope isolation, and asymmetric Metal FP16 tiled matmul CPU-oracle tests | Optional post-creation `MTLResource.allocatedSize` settlement and separate residency authority; device-loss quarantine/recovery; asynchronous and multi-device partitioning/scheduling; direct telemetry; retained performance evidence; and native support matrices |
