...
| Platforms | Native macOS development-host evidence, including the 49-death ActionOutbox POSIX recovery campaign and an on-demand Metal diagnostic-readiness gate; affected-path verification that selects only required host/OS gates and constructs one shared install-plus-test DAG per selected target; full opt-in build and test-compile gates for Linux x86_64/AArch64 musl, Windows x86_64 GNU, and FreeBSD x86_64; a bounded Linux available-memory adapter implementation with native retention still pending; exported package modules; compile-time adapter-availability inventory; read-only POSIX/Windows model-file mapping; portable process-ID and forced-termination fixtures; compile-only core probes for Android and iOS AArch64 | Retain addressable native Metal readiness output and named device coverage; split named core/CPU/durable/device/host-tool profiles further; run native Linux/Windows/FreeBSD CPU, observer, mapping, and recovery campaigns; implement the Windows durable-file adapter; finish clock, telemetry, and packaging adapters; then add mobile and reduced edge profiles |
