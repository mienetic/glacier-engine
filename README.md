...
- **Bounded concurrent transport.** The Phase F1 runtime
  ...
  ownership. This is implementation plus native-loopback correctness evidence,
  not a Phase F1 real-process, native-load/performance, GPU, or native
  foreign-OS result. Phase F1 concurrent serving is explicitly unsupported on
  Windows today: the entrypoint returns
