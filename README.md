...
- **Bounded loopback HTTP/1.1.** An experimental serial socket adapter exposes
  ...
  at zero until committed progress exists. Aggregate inspection reports
  convergence without claiming per-connection final outcome or peer delivery.
...
| AI runtime | CPU execution with optional macOS Metal kernels; portable conversion and prepared images; package, tokenizer, and raw-input admission; process-local and durable fixed-output text execution; continuation and acknowledged delivery; bounded unary HTTP serving with exact peer send-close classification and lifecycle accounting; fixed-worker transport correctness; dense reranking, embedding, classification, and retrieval fixtures; and shared scheduling, provider, media, and evidence planes | Retain longer and repeated production-model captures on reproducible native machines, publication-eligible Linux, separately declared GPU campaigns, forced-death and durable serving recovery, committed-token streaming, bounded EOS serving, broader model/package profiles, stable APIs, and distribution |
| Serving | Experimental package-bound process-local unary kernel with fixed capacities, retained idempotency records, hidden partial output, cancellation, stale-handle fencing, fail-stop, and zero-ownership close; a serial loopback-only HTTP/1.1 socket and retained bounded client implement one strict non-streaming JSON profile; R1k-b8 Phases A-D plus E2b add a managed child lifecycle, fenced connection/work leases, receive and full-request monotonic deadlines, drain/reset cancellation, bounded interruptible response writes, exact phase-specific evidence, same-child recovery, and zero-ownership close; hosts can select monotonic cancel-active or finish-published drain and inspect a versioned initiation receipt whose resumable count remains zero; Phase F1 adds `1..16` transport workers, a `1..64` FIFO of already accepted connections, passive kernel-backlog backpressure, shared deadline supervision, fenced one-owner handoff, exact snapshot conservation, joined drain, four retained deterministic same-process native-loopback scenarios, and four retained native POSIX child-process profiles covering queued receive and full-request deadlines, simultaneous drain, exact slot reuse, stale-owner rejection, fail-closed queued/running cleanup, event/cause conservation, joined threads, and final zero connection, Service, Scheduler, and Bank ownership, while model execution remains serialized; the same roots retain exact application causes with Service-validated Scheduler event identity, managed owner decoration, and no published HTTP `WorkIdentityV1` | Add per-connection final drain settlement and committed continuation binding; collect longer/repeated native-load evidence using HTTP first-byte rather than streaming first-token latency; forced-death and durable restart campaigns, committed-token streaming, authentication, TLS, quota, GPU serving evidence, and native Windows/FreeBSD serving proof remain open |
