...
- **Read-only token-transaction inspection.** A Python-first inspector replays
  one bounded strict-B4 TokenTxn journal against a complete caller-supplied
  expectation. Its default report omits token IDs; explicit disclosure renders
  all four 64-token lane outputs. Verification establishes integrity and
  composition relative to that expectation, not authenticity, provenance, or
  runtime authority.
...
- **Bounded loopback HTTP/1.1.** An experimental serial socket adapter exposes
  ...
  drive quanta, exact receive-side FIN classification, an explicit admitted-work
  TCP send-half-close policy, opt-in response-side FIN probing,
  response-ready drain cancellation before the first write, bounded
  interruptible response sends, and an optional full-request
  timeout whose accept-origin boundary ends at local response write completion
  with phase-specific outcome evidence. Later response retirement or observer
  cleanup cannot change the already-selected timeout-versus-local-completion
  winner. A timeout claimed before completion publication remains the
  lifecycle winner even if already-flushed bytes leave the transport outcome
  as `write_completed`. Receive-side FIN classification distinguishes zero
  request bytes, a partial request head, and a partial declared request body.
  The first two close without a response; the partial-body case returns the
  canonical HTTP 400 invalid-request response. Public
  `PeerSendClosePolicyV1` preserves the response by default;
  a managed host may opt into `abandon_after_complete_request` only when its
  transport contract treats FIN after a complete request as abandonment.
  Distinct counters retain FIN observation and exact-work cancellation at
  `request_admitted`. Only the opt-in adds response-side FIN probes: zero-byte
  `cancelled_before_write` at `response_ready`, and requested plus effective
  `cancelled_during_write` after an exact one-byte prefix at
  `response_writing`. A separate final-send control delivers real peer
  `shutdown(.send)` only after the retained `response_written` barrier. It
  performs no post-send socket probe, advances no FIN observation or
  cancellation counter, and keeps local `write_completed` as the winner.
  Hosts can now initiate versioned `cancel_active` or `finish_published`
  drain. The finish policy closes new admission while allowing exact work
  published before the lifecycle boundary and already materialized responses
  to settle; it can later be escalated to cancellation but never downgraded.
  The initiation receipt conserves every captured connection across
  abort/finish/reject/preexisting-stop decisions, reports exact active-work
  cancellation evidence and pending settlement, and fixes resumable selection
  at zero until committed progress exists. The first receipt also opens a
  bounded settlement session whose retained members bind the exact transport
  owner, first phase, initial and latest policy decision, policy revision, and
  work-stop evidence. A member advances from active to transport-closing and
  becomes immutable only after both logical retirement and explicit native
  socket-close confirmation. Its final receipt separates lifecycle status from
  the terminal cause, retains exact completion/failure counter ordinals and
  local-write evidence, and never upgrades local completion into peer delivery.
  The existing native process fixture now covers finish-published followed by
  cancel-active escalation through revision two and clean final settlement.
  Resumable remains zero; committed continuation authority is still open.
  Existing drain entry points keep their original cancel behavior.
  Non-loopback serving, authentication, TLS, streaming, model-kernel
  preemption, peer delivery acknowledgement, durable restart, native
  foreign-OS behavior, and performance evidence remain open.
...
The Phase B receive-drain, Phase C monotonic receive-timeout, Phase D
admitted-work drain, and Phase E1 reset/response-ready plus FIN-policy cases
for zero-byte, partial-head, and partial-body receive, `request_admitted`,
opt-in `response_ready` and `response_writing`, and final-send local completion
reuse that same dual-mode executable and those same targets; they add no
compile root or CI load profile. A unary-kernel implementation change selects
...
Inspect one bounded TokenTxn journal against a separately trusted expectation
...
The default report redacts token IDs. Add `--reveal-token-ids` only when the
report destination may receive the complete `4 x 64` token matrix. Both input
files already contain token IDs, and default roots and epochs remain
correlatable metadata. The caller remains responsible for obtaining the
expectation through a trusted workflow; matching it does not establish
authenticity or grant authority. See
[Token Transaction Inspector](docs/TOKEN_TXN_INSPECTOR.md).
...
| AI runtime | CPU execution with optional macOS Metal kernels; portable conversion and prepared images; package, tokenizer, and raw-input admission; process-local and durable fixed-output text execution; continuation and acknowledged delivery; bounded unary HTTP serving with exact peer send-close classification, lifecycle accounting, selectable drain policy, aggregate initiation inspection, and immutable per-connection settlement after socket-close confirmation; fixed-worker transport correctness; dense reranking, embedding, classification, and retrieval fixtures; and shared scheduling, provider, media, and evidence planes | Retain longer and repeated production-model captures on reproducible native machines, publication-eligible Linux, separately declared GPU campaigns, durable serving recovery, committed-progress drain continuation, committed-token streaming, bounded EOS serving, broader model/package profiles, stable APIs, and distribution |
| Serving | Experimental package-bound process-local unary kernel with fixed capacities, retained idempotency records, hidden partial output, cancellation, stale-handle fencing, fail-stop, and zero-ownership close; a serial loopback-only HTTP/1.1 socket and retained bounded client implement one strict non-streaming JSON profile; R1k-b8 Phases A-D plus E2b add a managed child lifecycle, fenced connection/work leases, receive and full-request monotonic deadlines, drain/reset cancellation, bounded interruptible response writes, exact phase-specific evidence, same-child recovery, and zero-ownership close; hosts can select monotonic cancel-active or finish-published drain, inspect a versioned initiation receipt, and retain exact per-connection final settlement only after logical retirement plus transport-close confirmation; drain resumable remains zero; Phase F1 adds `1..16` transport workers, a `1..64` FIFO of already accepted connections, passive kernel-backlog backpressure, shared deadline supervision, fenced one-owner handoff, exact snapshot conservation, joined drain, four retained deterministic same-process native-loopback scenarios, and native POSIX child-process profiles covering queued receive and full-request deadlines, simultaneous drain, finish-to-cancel escalation, exact slot reuse, stale-owner rejection, fail-closed queued/running cleanup, event/cause conservation, joined threads, and final zero connection, Service, Scheduler, and Bank ownership, while model execution remains serialized; the same roots retain exact application causes with Service-validated Scheduler event identity, managed owner decoration, and no published HTTP `WorkIdentityV1` | Bind any nonzero drain-resumable result to committed continuation authority; collect longer/repeated native-load evidence using HTTP first-byte rather than streaming first-token latency; forced-death and durable restart campaigns, committed-token streaming, authentication, TLS, quota, GPU serving evidence, and native Windows/FreeBSD serving proof remain open |
| Peer send-close | Exact receive-side zero-byte/partial-head/partial-body classification; default response preservation after a complete request; explicit managed opt-in cancellation at admitted-work, response-ready, and response-writing checkpoints; and a final-send control that closes the observation window after local write completion without a post-send probe | Native foreign-OS evidence and higher-level peer acknowledgement remain open; local `response_written` evidence does not prove peer receipt or processing |
