...
- **Bounded loopback HTTP/1.1.** An experimental serial socket adapter exposes
  ...
  child-process lifecycle now supports exact drain cancellation and an
  optional 1-millisecond-to-60-second monotonic receive deadline for partial
  headers and bodies while keeping timeout evidence separate from drain.
  Non-loopback serving, authentication, TLS, streaming, full-request
  cancellation, durable restart, and performance evidence remain open.
...
Managed server-process changes run `unary-server-process-test`, with
`unary-server-process-compile` reserved for retained-target compile evidence.
The Phase B drain cases and Phase C monotonic receive-timeout cases reuse that
same dual-mode executable and those same targets; they add no compile root.
Kernel or shared server implementation changes compose the service, HTTP, and
