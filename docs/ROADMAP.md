...
| AI runtime | Mixed prototype/integrated planes; R0 registry complete | CPU execution, an optional macOS Metal kernel path, prepared `.glrt` images, admission, scheduling, token publication, continuation, provider control plane, canonical model-family contracts, shared stateless/stateful lifecycles, scheduler-coupled media transactions and typed perception results, restart-safe generated-media output verticals, exported Zig package modules, an experimental C verifier, and a deterministic eight-profile retained-reference support registry/inspector with C, Python, and Rust queries | R1 text-path binding behind the common plan, stable API policy, product-specific build profiles, physical resource integration, and distribution |
