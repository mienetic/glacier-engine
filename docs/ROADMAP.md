...
| End-to-end user path | One ordinary-package-aware process-local raw-text command plus a composed synthetic durable recovery fixture and separate read-only result inspector; canonical work is AI Runtime R1 | `package-model` joins supported Safetensors conversion, prepared CPU image, exact tokenizer/license identity, and an 896-byte manifest-plus-representation bundle; `text-run --package` compares the embedded receipt with the actual GLRT identity before publishing token IDs transactionally in process; the recovery fixture separately carries package/tokenizer/raw-input identities through public one-step POSIX bootstrap/source/target APIs; fixed output count one has a sink-free direct-terminal path with a bounded four-boundary process-death smoke, while counts `2..64` use one runtime-capacity acknowledged store; public filesystem readers validate direct or acknowledged committed output without a writer lease | Connect package-aware execution to checked durable output and fresh-process recovery without duplicating the protocol; retain exhaustive storage-fault/power-loss and non-POSIX native evidence; broaden tokenizer/model/GPU package profiles; and remove silently selected stubs |
...
- [ ] Connect package-aware execution to the checked committed-output views and
  fresh-process continuation without duplicating the recovery protocol.
...
13. ~~Expose the retained filesystem composition through public Zig entry
    ...
    experimentally. Checked durable `text-run`/serving integration, exhaustive
    storage/power-loss evidence, broader tokenizer/model/GPU package profiles,
    and non-POSIX native evidence remain open.
