...
| AI runtime | Mixed prototype/integrated planes; R0 registry complete, R1a/R1b/R1c/R1d/R1e/R1f/R1g/R1h-a/R1h-b/R1i/R1j/R1k-a/R1k-b1/R1k-b2/R1k-b3 plus the durable handoff slice integrated experimentally, and the R2 generic reranker and normalized embedding fixtures integrated | CPU execution, an optional macOS Metal kernel path, portable `.glacier` conversion and prepared `.glrt` images with recoverable POSIX publication and exact predecessor/successor convergence, an exact-identity serial greedy prepared-text session with plan-derived atomic start, Common Model Contract request-profile binding, a request-independent package manifest and separate prepared-representation identity, a strict canonical UTF-8 byte-tokenizer and fixed raw-input binding, one retained-fixture-only process-local `text-run` golden path, an additive durable raw-input archive with fresh-process re-tokenization, total-versus-request claim projection for shared read-only artifact residency, fixed-length terminal results, canonical non-terminal state, same-process rebind, canonical successor evidence, receipt-funded restored activation at sequence `N`, a canonical generation-one source replay contract, compatible five- and six-object restart archives with exact source exit, an idempotent POSIX result sink joined to acknowledged generation-three-through-five progress under a combined 49-point real-process-death campaign, public experimental one-step bootstrap/source/target durable-runtime APIs used by that campaign, a sink-free direct-terminal generation-one-to-two path for fixed output count one with a separate four-boundary real-process-death smoke and independent oracle, one concrete durable store accepting runtime acknowledgement capacities `0..63` without per-capacity production-runtime monomorphization, public read-only committed-output filesystem APIs, a metadata-first result inspector for aligned or exactly-one-ahead sink state, a download-free exact-integer dense-tensor reranker with canonical ranked items, and an exact Q30 L2 dense-embedding result; plus admission, scheduling, continuation, provider, media, package, C verifier, and retained-reference inspector surfaces | Add an ordinary-model package producer and integrate checked output into ordinary `text-run` and future serving surfaces; then complete exhaustive direct-terminal storage-fault and power-loss evidence, request-shape accounting, variable-length results, stable APIs, production fixtures, non-POSIX native evidence, broader native validation, GPU/device continuation, physical resource integration, and distribution |
...
| Weight paging | Prototype with sealed conversion | Strict portable layout/geometry/CRC admission, bounded per-page transformation, exact conversion identities, and recoverable target publication | Whole-model tensor schema, real generation integration without eager duplicate weights, broader source dtypes, and production evidence |
| End-to-end user path | One bounded process-local raw-text command plus a composed synthetic durable recovery fixture and separate read-only result inspector; canonical work is AI Runtime R1 | `gen-fixture → convert → prepare → text-run` joins strict raw UTF-8 identity to CPU execution, transactional in-memory token publication, terminal evidence, and zero logical ownership; the recovery fixture carries the same package/tokenizer/raw-input identities through public one-step POSIX bootstrap/source/target APIs; fixed output count one has a sink-free direct-terminal path with a bounded four-boundary process-death smoke, while counts `2..64` use one runtime-capacity acknowledged store; the public filesystem readers validate direct or acknowledged committed output without a writer lease | Add the package producer needed to unify these APIs under one documented command, then integrate checked output into ordinary rendering, retain exhaustive storage-fault/power-loss and non-POSIX native evidence, and remove silently selected stubs |
...
Status: **one supported process-local golden path; no supported durable golden
path yet**.
...
- [x] Add read-only committed-output inspection over the selected durable
  ...
  state, hides output by default, and requires explicit lossless byte
  disclosure. It does not complete ordinary `text-run` or serving rendering.
...
- [ ] Add an ordinary-model package producer, then integrate the checked
  committed-output views into `text-run` without duplicating the recovery
  protocol.
...
12. ~~Add a read-only committed-output view over the selected checkpoint and
    ...
    payload bytes unless `--reveal-output` is explicit. Ordinary command and
    serving rendering remain open.
13. ~~Expose the retained filesystem composition through public Zig entry
    ...
    convergence, and a zero-step audit. The package producer, CLI/serving
    integration, exhaustive storage/power-loss evidence, and non-POSIX native
