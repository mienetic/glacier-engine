...
| Native unary load | Two fixed manual CPU/loopback profiles | One opt-in target reuses the managed-process artifact and compile root for 8 warmup plus 64 measured requests over one real native child. The default profile completes all requests; the retained-record-capacity profile completes 32 and rejects 32 measured requests with exact HTTP 429 `service_capacity` evidence. Both retain request/work/transport correlation, HTTP first-byte or terminal-response observations, captured machine state, joined cleanup, and zero ownership | Add queued-timeout and explicit open-loop profiles, production models, repeated eligible captures, publication-eligible native Linux, and separately declared GPU campaigns |
...
- [ ] Broaden Phase F1 load evidence beyond the two fixed synthetic CPU
  profiles: queued-timeout and broader overload campaigns, longer and repeated
  captures, production-model profiles,
  publication-eligible native Linux environment evidence, and separately
  declared GPU campaigns. Do not infer first-token latency, fairness,
  completion order, parallel model execution, physical CPU parallelism, GPU
  performance, or another operating system's behavior from either fixed
  profile.
