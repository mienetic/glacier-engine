...
- [x] Deterministic read-only runtime support registry and inspector generated
  from eleven retained exact-integer adapter profiles, with append-only mask
  ...
  without changing indices 0 through 9. Registration is not execution and no
  host backend is probed.
- [x] Download-free R2 generic classification slice with exact
  ...
  This completes only the generic fixture; retrieval composition, production
  adapters, probabilities/calibration/labels, GPU and native multi-OS
...
- report an explicit compatibility matrix generated from tests; complete for
  the eleven append-only retained-reference profiles and required for every
