...
| Native unary load | Three fixed manual CPU/loopback profiles | One opt-in target reuses the managed-process artifact and compile root. The default profile retains 8 warmups plus 64 measured completions; the retained-record-capacity profile retains 32 completions and 32 HTTP 429 `service_capacity` measured outcomes; and the closed-loop queued-receive-timeout profile rotates 8 measured epochs to `2 completed / 6 timed_out` per flow. The profiles retain their declared request/work/transport correlation, response or timeout evidence, captured machine state, joined cleanup, and zero ownership | Add broader overload profiles, production models, repeated eligible captures, publication-eligible native Linux, and separately declared GPU campaigns |
...
target adds one fixed all-completed CPU/loopback capture without changing the
core-first runtime boundary. Table entries that say to run native-load evidence
next refer to broader overload/rejection, production-model, repeated-machine,
publication-eligible Linux, and GPU campaigns, not to a missing runtime entry
point or correctness gate.
