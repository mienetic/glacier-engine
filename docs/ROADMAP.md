...
| End-to-end user path | One bounded process-local raw-text command plus a composed synthetic durable recovery fixture and separate read-only result inspector; canonical work is AI Runtime R1 | `gen-fixture → convert → prepare → text-run` joins strict raw UTF-8 identity to CPU execution, transactional in-memory token publication, terminal evidence, and zero logical ownership; the recovery fixture carries the same package/tokenizer/raw-input identities through public one-step POSIX bootstrap/source/target APIs; its concrete durable store accepts runtime acknowledgement capacities `0..63`, while the current source/target transition protocol covers fixed output counts `2..64`; the public filesystem inspector admits aligned or exactly-one-ahead committed output without a writer lease | Add the direct-terminal fixed-output-count-one path and package producer needed to unify these APIs under one documented command; then integrate checked output into ordinary rendering, retain non-POSIX native evidence, and remove silently selected stubs |
...
- [ ] Add the direct-terminal fixed-output-count-one path and an ordinary-model
  package producer, then integrate the checked committed-output view into
  `text-run` without duplicating the recovery protocol.
...
13. ~~Expose the retained filesystem composition through public Zig entry
    ...
    monomorphization. The current transition protocol covers fixed output counts
    `2..64`; the direct-terminal fixed-output-count-one path, package producer,
    CLI/serving integration, and non-POSIX native evidence remain open.
