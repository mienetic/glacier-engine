...
| End-to-end user path | Ordinary-package-aware process-local and checked durable fixed-output `1..64` raw-text execution with fresh-process retry, plus an experimental serial loopback HTTP/1.1 route and retained client for the fixed-output process-local profile; canonical work is AI Runtime R1 | `package-model` joins supported Safetensors conversion, prepared CPU image, exact tokenizer/license identity, and an 896-byte manifest-plus-representation bundle; `text-run --package` compares the embedded receipt with the actual GLRT identity before process-local publication or durable execution; count one uses the sink-free `1/1 -> 2/1` route, counts `2..64` use the acknowledged source/target chain with capacity `N - 1`, and both render only a checked read-only view after ownership closes; `GET /v1/models` and bounded `POST /v1/chat/completions` map one strict non-streaming request onto the same process-local kernel while the client validates response correlation | Add a separately managed server lifecycle, streaming, and early-EOS or fewer-than-admitted output; retain exhaustive storage-fault/power-loss and non-POSIX native evidence; broaden tokenizer/model/GPU package profiles |
| Serving interface | Experimental process-local unary kernel plus bounded serial loopback HTTP/1.1 route and retained client | Bounded model binding, shared Scheduler/Bank lifecycle, request identity, process-local idempotency replay/conflict, capacity rejection, cancellation without partial output, stale-handle fencing, retained fixed responses, fail-stop, zero-ownership close, strict one-user JSON, logical-deadline headers, strict UTF-8 response rendering, and client-side schema/correlation validation | Separately managed server process, disconnect/timeout/overload/drain/restart campaigns, committed-token streaming, authentication, TLS, quota, and transport security |
...
HTTP/1.1 route, and retained client integrated**.
