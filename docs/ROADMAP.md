...
| Token publication | Integrated; read-only inspection prototype | Contiguous/paged transactions plus exact-once logical next-token publication in the retained model-free fixture after natural exit and every checkpoint root-switch death phase; a Python-first inspector replays one bounded strict-B4 journal against a complete caller-supplied expectation while omitting token IDs by default | Uninterrupted/resumed production comparison, trusted expectation issuance, broader transaction profiles, and operational retention/disclosure policy |
...
| AI runtime | Mixed prototype/integrated planes; R0 registry complete, experimental R1 serving and durable handoff slices integrated through Phase F1 correctness, and R2 reranker, embedding, classifier, and retrieval fixtures integrated | CPU execution with optional macOS Metal kernels; portable conversion and prepared images; package/tokenizer/raw-input admission; process-local and durable fixed-output text execution; continuation and acknowledged delivery; bounded unary HTTP serving with exact peer send-close classification, lifecycle accounting, selectable drain policy, aggregate initiation inspection, and immutable per-connection settlement after socket-close confirmation; fixed-worker transport correctness; native POSIX managed-drain evidence including finish-to-cancel escalation; a nine-victim native POSIX forced-death matrix over every owned connection phase; exact post-parse rejection observations; and shared scheduling, provider, media, package, C-verifier, and evidence surfaces | Production-model and longer repeated native captures, publication-eligible Linux, separate GPU campaigns, native foreign-OS serving, committed-progress drain continuation, durable restart, committed-token streaming, early-EOS serving, broader model/package profiles, physical telemetry, stable APIs, and distribution |
| Native unary load | Four fixed manual CPU/loopback profiles | One opt-in target reuses the managed-process artifact and compile root. The default profile retains 8 warmups plus 64 measured completions; the retained-record-capacity profile retains 32 completions and 32 HTTP 429 `service_capacity` measured outcomes; the closed-loop queued-receive-timeout profile rotates 8 measured epochs to `2 completed / 6 timed_out` per flow; and the scheduled transient-pressure profile retains 16 baseline, 32 pressure, and 16 recovery client launches whose timing is independent of request outcomes. Every profile retains its declared request/work/transport correlation, response or timeout evidence, captured machine state, joined cleanup, and zero ownership | Add longer/repeated captures, natural or application-driven overload, production models, publication-eligible native Linux, and separately declared GPU campaigns |
| Managed peer send-close | Receive through final local send integrated | Exact zero-byte, partial-head, and partial-body receive classification; default completed-request response preservation; explicit managed opt-in cancellation at admitted-work, response-ready, and response-writing checkpoints; and final-send local-completion evidence all reuse the existing unary roots | Native foreign-OS evidence and higher-level peer acknowledgement remain open; local completion is not peer receipt or processing |
...
| Model-family breadth | Text-generation prototype, typed vision/audio/temporal-video encoders, a generic exact-integer dense-tensor reranker with canonical batch/rank/tie policy, a generic exact Q30 L2 dense-embedding fixture, a generic exact-integer dense-tensor classifier with authenticated class maps and deterministic class-ordinal ties, a tenant-filtered fixed-corpus exact retrieval fixture with authenticated index/query/top-k identity, fresh-process stateful transcript and VFR video models, exact word/speaker annotations, video-segment results, canonical video timelines, exact audio/video result links, exact latent continuation, generated-image publication, acknowledged generated-PCM/video publication, atomic generated-output checkpoints, exact encoded-payload archive composition, bounded multi-output registry continuity, canonical typed producer admission, exact deterministic producer-transition replay, validated bounded lossless delivery profiles for retained image/audio/video shapes, a typed pure-tool transaction fixture, a durable POSIX external-action handoff fixture, and a bounded same-process fake dispatch/status authority; other families gated | Shared artifact/plan/result wires, explicit support records, reusable stateless/stateful lifecycles, fresh-Bank retained-state restore, tensor, retrieval, provider, media, tool-authorization, ActionOutbox, and evidence building blocks | Optional retrieval-to-reranker handoff, durable or approximate index strategies, production classification and other model adapters, richer multimodal fusion, live provider-backed and OS-isolated dispatch/status adapters, production generative-media adapters and broader formats, additional replay profiles, agents, and specialized families |
...
| Durable provider evidence | Integrated; bounded two-mode inspection prototype | Crash-recoverable journal, compact evidence join, and one deterministic read-only inspector with byte-identical outer-only output plus optional caller-supplied nested replay and exact cross-wire composition | Privacy-safe export, retention, operational policy, and live credential-isolated adapters |
...
| End-to-end user path | Package-aware process-local and checked durable fixed-output `1..64` raw-text execution with fresh-process retry, plus an experimental loopback HTTP/1.1 route, retained client, bounded managed child-process acceptance, and selectable drain initiation with exact final settlement | `package-model` binds supported Safetensors conversion, prepared CPU image, tokenizer/license identity, and exact prepared-representation evidence. The same process-local kernel now retains receive-side peer send-close classes, default completed-request response preservation, explicit opt-in checkpoint cancellation, final-send local completion, phased deadlines, native managed-drain phase evidence plus finish-to-cancel escalation, same-child liveness, concurrent transport correctness, immutable settlement after logical retirement and native close, a nine-victim forced-death phase matrix with one clean fresh successor, and four manual native CPU load profiles including scheduled transient pressure | Add committed-progress drain continuation and durable restart; retain longer/repeated production-model captures, publication-eligible Linux, separately declared GPU campaigns, streaming and early-EOS output, exhaustive storage/power-loss evidence, native multi-OS execution, and broader tokenizer/model/GPU package profiles |
| Serving interface | Experimental process-local unary kernel, bounded loopback HTTP/1.1 route and retained client, managed child-process evidence through final local send, Phase F1 fixed-worker concurrent transport, selectable versioned drain initiation, aggregate inspection, immutable per-connection settlement, independent native POSIX managed-drain and forced-death phase matrices, and one manual native CPU loopback load target with four fixed profiles | Exact zero-byte/partial-head/partial-body receive classes; default preserve plus explicit opt-in cancellation at admitted-work, response-ready, and response-writing checkpoints; final-send `write_completed` with no post-send probe; bounded drain settlement at every owned phase, including both admitted-work substates; exact owner/phase/decision/revision retention; separate logical retirement and native-close confirmation; native finish-to-cancel escalation; phased deadlines; a bounded `1..16`-worker/`1..64`-pending transport with shared watchdog and exact conservation; checked abrupt child termination over all eight owned phases; application-rejection observations; callback-order regression; and scheduled transient-pressure evidence. Model execution remains serialized | Add committed-progress drain continuation, durable restart, committed-token streaming, authentication, TLS, quota, native foreign-OS serving, and separate GPU campaigns. Current drain evidence is non-resumable and local close is not peer receipt; forced-death evidence is not durable request recovery or idempotency; local completion is not first-token latency, fairness, or a performance-superiority claim |
...
The Phase F1 serving row is the current transport snapshot. Core-first serving
now retains exact post-parse service-capacity and Scheduler-rejection
observability through the existing roots. The separate manual native-load
target adds fixed all-completed, retained-capacity, queued-timeout, and
scheduled transient-pressure CPU/loopback profiles without changing the
core-first runtime boundary. Remaining native-load work means longer and
repeated captures, natural or application-driven overload, production models,
publication-eligible Linux, and separately declared GPU campaigns—not a
missing runtime entry point or correctness gate.
...
- [x] Deterministic read-only runtime support registry and inspector generated
  from twelve retained exact-integer adapter profiles, with append-only mask
  ...
  [fixture-authoring guide](RUNTIME_SUPPORT_INSPECTOR.md). Profile 10 appends
  the bounded generic [dense-tensor classifier](DENSE_TENSOR_CLASSIFIER.md);
  profile 11 appends exact fixed-corpus
  [dense-tensor retrieval](DENSE_TENSOR_RETRIEVAL.md) without changing indices
  0 through 10. Registration is not execution and no host backend is probed.
- [x] Download-free R2 generic classification slice with exact
  `i16 × i8 → i64` class scores, authenticated batch/class maps, canonical
  no-normalization/descending/class-ordinal tie policy, deterministic winners,
  and stateless direct/scheduled atomic publication with zero final ownership.
  This completes only the generic fixture; production adapters,
  probabilities/calibration/labels, GPU and native multi-OS
  execution, quality, performance, and provider-token evidence remain open.
- [x] Download-free R2-to-R5 exact fixed-corpus retrieval composition with an
  authenticated index generation, corpus batch and tenant-visibility maps,
  normalized Q30 corpus/query matrices, one canonical top-k policy, checked
  `i128` dot accumulation, ties-to-even Q30 scores, visibility-before-ranking,
  corpus-ordinal ties, and a retrieval-specific fixed-capacity result wire.
  Direct and scheduled execution independently reconstruct candidates before
  atomic publication and close with zero ownership. This is not ANN,
  persistence, production retrieval quality, a reranker handoff, GPU/native
  multi-OS execution, performance, provider routing, or token-saving evidence.
...
- [x] Collapse focused dense-tensor verification into one Zig test artifact
  ...
  the reranker, embedding, and retrieval target names as compatibility targets
  backed by the shared compiled artifacts and keep their focused oracles
  distinct. Route Python-test-only tensor, embedding, and retrieval changes
  independently, route embedding/classifier/retrieval changes through the
  family DAG, and add the runtime-support inspector only for adapter-core
  changes.
...
- [x] Separate the retained-target main runtime closure from benchmark
  promotion. General core, CPU, and model changes compile `install` plus
  `test-compile` in one invocation per selected target; bulk benchmark staging
  and the standalone benchmark consumer closure stay with benchmark and build
  changes, manual matrix runs, and tagged releases. Device diagnostics already
  owned by `test-compile` remain in the main closure.
- [x] Make iterative host routing explicit per affected path class.
  ...
  plans avoid the generic Zig DAG; Zig build controls evaluate the graph
  without compiling a runtime artifact; interop fixtures, other build
  controls, generic portable compiled code, and conservative inputs retain the
  generic roots, while audited focused classes keep their named DAGs. Mixed
  generic and focused roots share one invocation. The native Metal compile
  frontier now follows only its explicit suite-consumer inventory, leaving
  eleven unrelated artifacts to their deliberate profile gates.
...
- [x] Optional provider evidence composition mode with every nested input
  supplied explicitly, bounded replay, exact cross-wire equality, deterministic
  raw-payload-free JSON, and authority fixed to false.
- [x] Python-first read-only token transaction inspector for the bounded
  strict-B4 journal, with a complete caller-supplied expectation, fixed input
  bounds, metadata-only default output, explicit token-ID disclosure, and a
  focused Python-only CI route. It verifies integrity and composition relative
  to the expectation, not authenticity or authority.
...
- [x] optional provider nested-composition verification with every required
  input supplied explicitly;
- [ ] privacy-safe provider evidence export, retention, and operational policy;
- LaneWeave timeline JSON;
- [ ] per-wave token transaction root explorer; the bounded inspector now
  exposes summary roots and per-lane output digests, while wave selection and
  trace rendering remain open;
...
- report an explicit compatibility matrix generated from tests; complete for
  the twelve append-only retained-reference profiles and required for every
...
- [x] Freeze one bounded JSON HTTP/1.1 R1 profile: `GET /v1/models`
  identifies the one loaded package-bound model and
  `POST /v1/chat/completions` accepts one strict UTF-8 `user` message,
  `stream=false`, and `max_tokens` from 1 through 64. Exact
  `Idempotency-Key`, `Glacier-Tenant-Key`, and optional
  `Glacier-Deadline-Tick` headers bind the request before the same unary
  kernel executes it. The serial socket rejects non-loopback binds and
  non-UTF-8 model output fails closed.
- [x] Add a retained bounded client for the frozen model-list, completion,
  error, and request-correlation profile. It accepts loopback numeric
  endpoints only, owns decoded protocol values, rejects redirects and encoded
  or persistent responses, and performs no automatic retry.
- [x] Add R1k-b8 Phase A managed child-process lifecycle evidence without
  changing the frozen HTTP profile or unary state machine. One nonzero process
  generation moves through `starting`, `ready`, `draining`, and `stopped`, with
  `failed` terminal on lifecycle error. An exact out-of-band `drain\n` command
  followed by EOF, or empty stdin EOF, closes completion admission before a
  loopback wake releases the blocked accept. The dual-mode process fixture
  proves invalid startup emits no ready frame, a malformed peer is isolated
  from later valid requests, exact replay remains process-local, clean close
  has zero active service requests and zero Bank ownership, and a fresh
  generation loading the same package retains model and output identity under
  a new idempotency key.
- [x] Add R1k-b8 Phase B receive-side drain cancellation without adding a
  second execution state machine or compile artifact. One active-connection
  lease is fenced by process generation, connection sequence, and native
  handle. Its exact phase begins at `receiving_head` and advances once to
  `request_head_received`. Drain closes completion admission before lifecycle
  state becomes `draining`, shuts down the active receive side while holding
  the lifecycle lock, and leaves the serving thread as the sole closer; that
  thread retires the fenced lease before closing the socket. The same
  real-process target holds partial-header and declared-length partial-body
  peers open in separate generations, drain-cancels each with one failed and
  zero completed connection, and proves zero service requests and Bank
  ownership.
- [x] Add R1k-b8 Phase C managed pre-admission receive timeout without a new
  compile root. `receive_timeout_ns = 0` disables it; valid nonzero values are
  1 millisecond through 60 seconds. One monotonic timer starts immediately
  after accept and does not reset after the HTTP head. Before every managed
  socket receive, the serving thread waits for readability against the
  remaining monotonic budget and recomputes that budget after each read, so
  incremental peer progress cannot extend the deadline. Only the exact fenced
  lease in `receiving_head` or `request_head_received` may be retired for
  timeout. Receipt of every required request byte advances to
  `request_received`. Timeout evidence is distinct from drain. A timeout leaves
  lifecycle `ready` and completion admission open. The timer joins before the
  serving thread retires and solely closes the socket. Separate real child
  generations expire partial-header and partial-body peers, send no response
  to them, serve a valid model-list request in the same child, then drain with
  zero service records and Bank ownership.
- [x] Add R1k-b8 Phase D fenced post-admission drain cancellation without a new
  compile root. After exact unary admission and before the first status or drive
  operation, the HTTP runtime publishes a sequence/full-handle-fenced
  active-work lease; managed lifecycle binds its sequence and handle digest to
  the exact active connection at `request_admitted`. Drain closes completion
  admission first, then cancels only that exact service handle. Repeated drain
  and cancellation-versus-terminal races retain an idempotent classified
  outcome, while newly cancelled work is counted separately from receive-side
  drain and timeout. One deterministic post-admission/pre-drive child barrier
  invokes ordinary drain twice and proves a correlated `request_cancelled`
  response with retry policy `never`, one accepted and completed connection,
  zero failed connections, zero receive-drain and receive-timeout signals, one
  work cancellation at `request_admitted`, zero active service requests, one
  terminal record, and zero Bank ownership. A second child drives the same
  one-token request terminal while its HTTP lease remains published, then
  proves drain preserves the successful completion and records no work
  cancellation. The serving thread remains the sole response writer and socket
  closer.
- [x] Add R1k-b8 Phase E1 bounded post-admission transport cancellation without
  a new compile root. At a managed checkpoint between drive quanta, a reset on
  the exact generation/sequence/handle-fenced connection cancels only its
  sequence/full-handle-fenced active work, keeps admission open, and records
  `peer_reset_connections`, `peer_reset_cancelled_work_connections`, and their
  exact last phases. After bounded response encoding, a separate response
  control advances the connection through `response_ready`,
  `response_writing`, and `response_written`. Drain may win at
  `response_ready`, before the first write, and records one exact
  `drain_cancelled_response_connections` outcome while the serving thread
  remains the sole writer and closer. Three deterministic real-process
  controls prove reset-after-admission, drain-at-response-ready, and normal
  completion from the same response-ready boundary. The completion control
  retains the oracle response, reaches `response_written`, and leaves every
  new cancellation counter zero. The controls isolate their counters, retain
  same-child liveness after reset, and close with zero active service requests
  and Bank ownership. This is between-quantum reset detection and
  cancellation-before-first-write evidence; this original control is not
  in-flight drive preemption, FIN accounting, slow or already blocked
  response-write interruption, full-request timeout, or peer delivery
  acknowledgement. The separate FIN-policy slices below add only their
  explicitly bounded phases.
- [x] Add the public admitted-work send-half-close policy without a new compile
  root. `PeerSendClosePolicyV1.preserve_response` is the default because TCP
  FIN closes only the peer's send half; a managed host may explicitly select
  `abandon_after_complete_request` when its application contract defines FIN
  after a complete request as response abandonment. The unmanaged listener
  rejects that opt-in instead of silently ignoring it. At the exact
  `request_admitted`
  checkpoint, `peer_send_closed_connections` counts the observation
  independently from `peer_send_close_cancelled_work_connections`, which
  increments only after the matching fenced handle is newly cancelled. Two
  deterministic POSIX child controls use real `shutdown(.send)`: the default
  control preserves work and receives the oracle response, while the opt-in
  control receives orderly EOF without response bytes, serves a healthy
  successor in the same child, and proves exact terminal records plus zero
  active Service/Bank ownership. Both reuse the existing process artifact,
  compile root, and bounded CI plan. This is admitted-work FIN evidence only,
  not native foreign-OS behavior or performance evidence.
- [x] Extend the opt-in FIN decision through the bounded response phases
  without a new compile root or CI load profile. At `response_ready`, a real
  child `shutdown(.send)` retains
  `peer_send_close_cancelled_response_connections = 1`, phase
  `response_ready`, outcome `cancelled_before_write`, and exactly zero response
  bytes. At `response_writing`, a one-byte send quantum reaches one real
  positive send before FIN; requested and effective response-write counters
  each advance once at `response_writing`, outcome
  `cancelled_during_write`, and the client retains exactly that one-byte
  prefix. Each child then completes a healthy successor and closes with two
  accepted connections, one completed, one failed, two terminal service
  records, and zero active Service/Bank ownership. The default policy performs
  no response-side socket probe, wait, or counter update and therefore
  continues normal response publication. Native foreign-OS behavior and
  production performance evidence remain open.
- [x] Add R1k-b8 Phase E2a bounded interruptible managed response writes
  without a new compile root. Each managed kernel send is nonblocking and
  bounded to a configurable `1..4096` bytes. The writer checks drain before a
  send, publishes a progress checkpoint after a real positive send, and checks
  drain when the socket reports `WouldBlock`; a drain requested from progress
  is observed before the next send. Separate counters retain drain requested while
  writing, effective cancellation at a later checkpoint, and an independent
  transport failure. A drain observed after the final successful send cannot
  relabel local completion: `write_completed` wins. Two deterministic
  real-process siblings use a one-byte quantum and meet after the first real
  one-byte send; one drains and records `cancelled_during_write`, while the
  other completes the oracle response. A focused native-loopback primitive
  separately saturates a real socket until `WouldBlock` and proves that
  cancellation checkpoint. This is bounded correctness evidence, not peer
  delivery acknowledgement or performance evidence. Native Windows and
  FreeBSD response-write behavior remains unproven.
- [x] Add R1k-b8 Phase E2b full-request elapsed timeout without a new compile
  root. `full_request_timeout_ns = 0` disables it; valid nonzero values are
  1 millisecond through 60 seconds. Its monotonic clock starts at accept and
  adjudicates the timeout-versus-local-completion winner through local response
  write completion. A timeout claimed before that completion remains the
  lifecycle winner even if already-flushed bytes leave the transport outcome
  as `write_completed`; once local completion wins, later response retirement
  or observer cleanup cannot relabel it. When both elapsed
  deadlines are enabled, `receive_timeout_ns` must be strictly shorter, so the
  receive boundary uniquely owns incomplete-head/body expiry. The full-request
  deadline is independent of wall time and of `Glacier-Deadline-Tick`, which
  remains an absolute logical Scheduler tick. Exact general signal, newly
  cancelled work, cancelled-before-write response, requested response-write
  stop, and effective response-write cancellation counters retain their last
  phases without relabeling drain, receive-timeout, reset, or transport
  failure. Expiry emits no new HTTP timeout response: before writing it sends
  no response bytes, and after writing starts it permits only an already-sent
  bounded prefix and no later send after timeout observation. Three
  deterministic same-child real-process cases meet at admitted/pre-drive,
  `response_ready`, and post-first-positive-send `response_writing` barriers,
  wait for lifecycle timeout evidence rather than sleep, verify the exact
  phase-specific subset, serve a valid model-list follow-up with admission
  still open, then drain with zero active service requests and Bank ownership.
  A completed local write that wins before timeout observation remains
  complete. Existing x86_64/AArch64 Linux musl, x86_64 Windows GNU, and
  x86_64 FreeBSD companions are multi-OS compile evidence only, not native
  timeout or socket proof.
- [x] Bind logical tenant, logical Scheduler deadline, idempotency key, and the
  `LaneWeave` receipt to the service request. The logical deadline remains
  distinct from Phase C receive time and Phase E2b full-request elapsed time.
...
- [x] Retain Phase F1 deterministic native-loopback correctness in the existing
  ...
  queued socket. Every behavior scenario checks snapshot/event conservation,
  ...
  dispatch observer callback before the enqueue observer callback while
  proving that both nonzero timestamps retain lifecycle-mutex order; it is not
  a fifth behavior scenario.
- [x] Retain a distinct Phase F1 real-process correctness campaign in the
  existing `unary-server-process-test` root, without a new artifact or build
  target. Four native POSIX child-process profiles use real loopback sockets:
  a queued receive timeout while active terminal work completes, followed by
  a healthy successor; a queued full-request timeout followed by a healthy
  successor; simultaneous two-caller drain over one active receive plus one
  queued socket; and stale-owner rejection after exact slot reuse, followed
  by fail-closed queued/running cleanup. The stale-owner profile deliberately
  corrupts only owner metadata as a white-box fault injection; its exact
  generation-mismatch rejection and cleanup use the production drain and
  failure paths. Every profile verifies aggregate event/cause conservation,
  unique contiguous event ordinals, thread joins, and final zero connection,
  Service, Scheduler, and Bank ownership. This is native POSIX child-process
  loopback correctness rather than simulation or load evidence. It is not
  native Windows concurrent-serving evidence. FIFO promises only accepted-
  queue dispatch order; it does not promise completion order, scheduler
  fairness, or parallel model execution.
- [x] Retain exact cause-specific post-parse application-rejection
  observability for service-capacity and Scheduler rejection through the
  existing unary HTTP and managed-process roots. The observer
  binds the canonical request digest and optional transport owner; managed
  serving replaces the owner with its fenced connection lease. Scheduler
  evidence projects a Service-validated event ABI, epoch, sequence, rejection
  reason, and event digest; it omits the complete replayable event. It runs
  after admission locks return and before response handling, publishes no HTTP
  `WorkIdentityV1`, leaves `RuntimeV1.next_work_sequence` unchanged, excludes
  non-rejection errors, and proves an application decision rather than peer
  delivery. Real-loopback and native child-process coverage retain exact causes
  and owners, unchanged HTTP work sequencing, and zero ownership without
  adding an artifact or compile root.
- [x] Retain an initial opt-in Phase F1 native CPU load capture without adding
  another Zig executable or compile root. `unary-server-native-load-test`
  reuses `glacier-unary-server-process-test` and runs one real native child
  process over real loopback sockets. Its fixed closed profile uses 8 warmup
  plus 64 measured requests, eight flows, two transport workers, and eight
  pending slots. The verified 79,780-byte envelope contains 72 exact
  client/server correlation sidecars, a 28-field zero-ownership closure, and
  an embedded Native Workload Report V1. It retains client arrival, accepted-
  FIFO enqueue, worker dispatch, admitted-work publication, HTTP first byte,
  decoded terminal response, final settlement through exact server
  transport/connection retirement, throughput, the exact all-completed outcome
  mix, balanced backpressure, joined threads, and final zero connection,
  Service, Scheduler, and Bank ownership. The real campaign is manual and is
  not selected by default tests or GitHub CI.
- [x] Add the fixed Phase F1 retained-record capacity profile to that same
  manual target and `glacier-unary-server-process-test` artifact. Selecting
  `-Dunary-server-native-load-profile=retention-capacity-v1` retains 72
  records: 8 completed warmup requests followed by 32 completed and 32 HTTP
  429 `service_capacity` measured outcomes. Every one of eight flows retains
  four completions and four rejections. The profile-specific sidecar and
  embedded Native Workload Report V1 bind each rejection to the exact request,
  `service_capacity`, `same_request_after_backoff`, HTTP 429, and response-byte
  count tuple plus its transport lifecycle and absence of service work. The
  rejected sidecar separately retains an opaque domain-separated producer
  digest of raw HTTP and a verifier-recomputed semantic root in its
  `output_sha256` slot. Raw bytes are not retained or reconstructable offline;
  completion binds both sidecar roots while the embedded W6 output root stays
  zero. Service terminal-record capacity is exactly 40; close proves zero
  connection, active service, Scheduler, and Bank ownership. This is
  retained-record capacity saturation, not transient or general overload,
  queued-timeout evidence, throughput superiority, production-model behavior,
  fairness, GPU behavior, or cross-OS evidence.
- [x] Add a deterministic Phase F1 queued receive-timeout profile to the same
  manual target and `glacier-unary-server-process-test` artifact without a new
  compile root. Selecting
  `-Dunary-server-native-load-profile=queued-receive-timeout-v1` completes 8
  sequential warmups before 8 measured epochs. Each epoch holds 2 running
  requests while 6 queued peers expire at their exact 2-second receive
  deadline, then completes the running pair. Flow rotation leaves each
  measured flow at `2 completed / 6 timed_out`. Exact closure requires
  enqueued `72`, dispatched/completed `24`, failed/queued-receive-timeout `48`,
  queue/running high-water `6/2`, pause/resume `8/8`, 24 completed Service
  records, event ordinal `184`, and zero live connection, Service, Scheduler,
  and Bank ownership.
  The verifier admits client arrival to timeout only in `2..4` seconds, enqueue
  to timeout at no more than 3 seconds, and timeout to peer-close/no-response
  transport settlement at no more than 1 second. Settlement requires zero
  response bytes and accepts a zero-length read or connection reset/abort.
  These are campaign-admissibility bounds, not general runtime latency
  promises. The fixed profile identity binds the configured 2-second timeout
  and all three caps.
  A timed-out W6 record has only arrival, terminal, and settlement,
  not-applicable correctness, and no Bank, work, dispatch, or output. Its outer
  record retains the client-observed canonical request root, an opaque raw
  outbound HTTP digest, exact enqueue/lease/`.queued_receive_timeout` evidence,
  and independently recomputable semantic, terminal, and completion roots.
  The queued connection emits only `.queued_receive_timeout`, never a separate
  `.retired` event; any `retired_*` sidecar fields are profile-defined aliases
  of that terminal event. It is never server-parsed. Request-to-lease
  association is deterministic single-outstanding client-plan/transmit
  correlation, not server-parsed request attestation. This is closed-loop
  queued-pressure evidence. It does not establish transient or general
  overload, generic queue latency, throughput superiority, fairness, physical
...
- [x] Add an explicitly scheduled Phase F1 transient-pressure ABI to the same
  manual target and process artifact without a new compile root. Selecting
  `-Dunary-server-native-load-profile=open-loop-transient-pressure-v1`
  completes 8 sequential warmups, then releases 64 ready client actors against
  one fixed schedule: 16 baseline launches at 25-millisecond steps, 32
  pressure launches at 5-millisecond steps, and 16 recovery launches at
  25-millisecond steps beginning 1,800 milliseconds after the anchor. Launches
  are independent of request outcomes. The verifier binds each client plan
  ordinal and phase to its scheduled offset, observed launch lateness, and
  transmit-complete time; requires the fixed gate, pressure, release, recovery,
  lateness, and recovery-slack boundaries; and requires complete settlement,
  joined cleanup, and zero ownership. This proves only explicitly scheduled
  client launches under deterministic, test-controlled native-loopback FIFO
  pressure and recovery. It does not establish server arrival time, throughput
  superiority, natural or general overload, a capacity limit, Scheduler
  saturation, rate tolerance, fairness or completion order, first-token latency,
  production-model behavior, physical CPU parallelism, GPU behavior, native
  foreign-OS behavior, or a recovery SLA.
- [ ] Extend Phase F1 load evidence beyond the four fixed synthetic CPU
  profiles with longer and repeated captures, natural or application-driven
  overload, production-model profiles, publication-eligible native Linux
  environment evidence, and separately declared GPU campaigns. Keep every
  campaign's schedule, observed timing, outcome, environment eligibility, and
...
- [x] Bind retained native-load context into one authoritative single-file
  ...
  durability.
- [x] Complete receive-side and final-send peer send-close correctness without
  a new compile root or CI load profile. Three native POSIX children classify
  real `shutdown(.send)` as zero request bytes, a partial request head, or a
  partial declared request body. A fourth delivers send-half-close only after
  the retained `response_written` barrier, performs no post-send socket probe,
  leaves every peer-send-close observation/cancellation counter at zero, and
  retains local `write_completed`. Every case serves a healthy successor and
  closes with zero Service/Bank ownership. These are local native POSIX
  correctness facts, not peer delivery, foreign-OS serving, or performance
  evidence;
- [x] Retain forced process-death test evidence for every owned connection
  phase without adding a compile root, load profile, or production API. The
  existing process artifact launches nine fresh native POSIX victims across
  the eight non-`none` phases; `request_admitted` has separate live-work and
  terminal-but-runtime-unretired cases. Each victim publishes its canonical
  pre-kill phase and ownership receipt before the supervisor validates the
  exact PID, sends PID-only `SIGKILL`, and requires signal-9 termination,
  stdout EOF without a graceful close frame, empty stderr, and the exact
  zero-byte, one-byte-prefix, or complete locally written client observation.
  Unique complete coverage precedes one clean fresh same-package successor
  with zero connection, Service, Scheduler, and Bank ownership. This is
  abrupt-death test evidence, not checkpoint-aware drain, graceful victim
  cleanup, durable request recovery, durable idempotency, or retry safety;
- [x] Retain an independent managed-drain phase matrix without adding a compile
  root, load profile, or production API. The existing process artifact drives
  nine real child/socket cases over `queued`, both partial receive phases,
  `request_received`, live and terminal-but-runtime-unretired
  `request_admitted`, `response_ready`, `response_writing`, and
  `response_written`. The two late no-action phases prove closed admission
  while preserving a complete correlated `service_closed` response or the
  already written completion; earlier phases prove exact queued retirement,
  receive shutdown, work cancellation, response cancellation, or bounded
  one-byte write interruption as selected by the current policy. Every case
  reaches stopped lifecycle state and zero Service, Scheduler, and Bank
  ...
  finish-all-in-flight guarantee;
- [x] Expose a versioned source-level host drain initiation contract without a
  ...
  ABI, successful delivery, or durable restart;
- [x] Retain a versioned per-connection drain settlement cohort without a new
  source file, compile root, or CI lane. The first initiation receipt opens
  exact members that bind transport owner, first phase, initial/latest policy
  decision, monotonic revision, and work-stop evidence. Logical retirement and
  explicit socket-close confirmation are separate; only both can publish an
  immutable final receipt with independent lifecycle status and terminal cause,
  exact completed/failed counter ordinal, retired-resource facts, and
  conservative close provenance. Concurrent queued sockets retain detached
  ownership until close, forced after-join evidence remains incomplete, stale
  session/member/owner inputs fail closed, and total members conserve against
  lifecycle counter deltas. The existing native process artifact adds an exact
  `finish_published` to `cancel_active` escalation through revision two and
  clean final settlement. Resumable remains zero and local completion is not
  peer delivery;
- **Next:** bind any nonzero drain-resumable result to an exact
  committed-progress identity and continuation authority;
...
- introduce authenticated identity, quota, and transport-security adapters only
  with explicit authority and nonclaims.
