...
| End-to-end user path | One bounded process-local raw-text command plus a composed synthetic durable recovery fixture; canonical work is AI Runtime R1 | `gen-fixture → convert → prepare → text-run` joins strict raw UTF-8 identity to CPU execution, transactional in-memory token publication, terminal evidence, and zero logical ownership; the recovery fixture carries the same package/tokenizer/raw-input identities through POSIX source/target restart and acknowledged delivery | Unify both surfaces into one documented command, add committed-token rendering, retained native evidence, and no silently selected stub |
...
- [ ] Add read-only committed-text inspection over the selected durable result.
