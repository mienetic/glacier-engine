...
| Continuation identity | Prototype | Capsule, object lifecycle, durable payloads, ownership/KV/runtime reconstruction, atomic immutable checkpoint generations, two-process resume, a canonical generation-one source replay contract with exact empty-sink admission, and a generation-two-through-five prepared-text chain with acknowledged one-token local POSIX target progress | Production model/tokenizer state, repeated/cancelled handoff evidence, native Linux execution, and durable lifecycle metadata |
| AI runtime | Mixed prototype/integrated planes; R0 registry complete, R1a/R1b/R1c/R1d/R1e/R1f/R1g/R1h-a/R1h-b/R1i/R1j/R1k-a/R1k-b1 plus the durable handoff slice integrated experimentally, and the R2 generic reranker and normalized embedding fixtures integrated | CPU execution, an optional macOS Metal kernel path, portable `.glacier` conversion and prepared `.glrt` images with recoverable POSIX publication and exact predecessor/successor convergence, an exact-identity serial greedy prepared-text session with plan-derived atomic start, Common Model Contract request-profile binding, a strict canonical UTF-8 byte-tokenizer and fixed raw-input binding, one retained-fixture-only process-local `text-run` golden path, total-versus-request claim projection for shared read-only artifact residency, fixed-length terminal results, canonical non-terminal state, same-process rebind, canonical successor evidence, receipt-funded restored activation at sequence `N`, a canonical generation-one source replay contract, a five-object restart archive with exact source exit, an idempotent POSIX result sink joined to acknowledged generation-three-through-five progress under a combined 49-point real-process-death campaign, a download-free exact-integer dense-tensor reranker with canonical ranked items, and an exact Q30 L2 dense-embedding result; plus admission, scheduling, continuation, provider, media, package, C verifier, and retained-reference inspector surfaces | Join verified raw-input identity to durable source/target recovery, add a stable package manifest and committed-token text rendering, complete request-shape accounting, variable-length results, generic classification and retrieval composition, non-blocking startup, stable APIs, production fixtures, native multi-OS validation, GPU/device continuation, physical resource integration, and distribution |
...
| End-to-end user path | One bounded process-local raw-text golden path plus separate durable recovery components; canonical work is AI Runtime R1 | `gen-fixture → convert → prepare → text-run` joins strict raw UTF-8 identity to CPU execution, transactional in-memory token publication, terminal evidence, and zero logical ownership; durable source/target recovery remains pre-tokenized and separate | Join the raw-input binding to restart-safe delivery, committed-token rendering, retained native evidence, and no silently selected stub |
...
- [ ] Extend that raw-input sequence through checkpoint selection, the durable
  local sink, fresh-process source/target recovery, and read-only committed-text
  inspection.
...
lifecycle, separate-process fixture, and generation-one replay boundary, and
