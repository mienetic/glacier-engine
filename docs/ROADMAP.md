...
| End-to-end user path | Package-aware process-local and checked durable fixed-output `1..64` raw-text execution with fresh-process retry, plus an experimental loopback HTTP/1.1 route, retained client, and bounded managed child-process acceptance | `package-model` binds supported Safetensors conversion, prepared CPU image, tokenizer/license identity, and exact prepared-representation evidence. The same process-local kernel now retains receive-side peer send-close classes, default completed-request response preservation, explicit opt-in checkpoint cancellation, final-send local completion, phased deadlines, an independent nine-case managed-drain phase matrix, same-child liveness, concurrent transport correctness, a nine-victim forced-death phase matrix with one clean fresh successor, and four manual native CPU load profiles including scheduled transient pressure | Longer/repeated production-model captures, publication-eligible Linux, separately declared GPU campaigns, a host-facing drain policy and receipt, durable restart, streaming and early-EOS output, exhaustive storage/power-loss evidence, native multi-OS execution, and broader tokenizer/model/GPU package profiles |
| Serving interface | Experimental process-local unary kernel, bounded loopback HTTP/1.1 route and retained client, managed child-process evidence through final local send, Phase F1 fixed-worker concurrent transport, independent native POSIX managed-drain and forced-death phase matrices, and one manual native CPU loopback load target with four fixed profiles | Exact zero-byte/partial-head/partial-body receive classes; default preserve plus explicit opt-in cancellation at admitted-work, response-ready, and response-writing checkpoints; final-send `write_completed` with no post-send probe; bounded drain settlement at every owned phase, including both admitted-work substates; phased deadlines; a bounded `1..16`-worker/`1..64`-pending transport with shared watchdog and exact conservation; checked abrupt child termination over all eight owned phases; application-rejection observations; callback-order regression; and scheduled transient-pressure evidence. Model execution remains serialized | Longer/repeated production captures, native foreign-OS serving, a stable host-facing drain policy and receipt, durable restart, committed-token streaming, authentication, TLS, quota, and separate GPU campaigns. Current drain evidence is phase-aware bounded cleanup, not resumable checkpoints or a finish-all-in-flight policy; forced-death evidence is not durable request recovery or idempotency; local completion is not peer receipt, first-token latency, fairness, or a performance-superiority claim |
