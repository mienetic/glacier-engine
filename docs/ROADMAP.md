...
| AI runtime | Mixed prototype/integrated planes; R0 registry complete and R1a experimental slice integrated | CPU execution, an optional macOS Metal kernel path, prepared `.glrt` images, an exact-identity serial greedy prepared-text session with adopted scheduling receipt and transactional KV/RNG/token publication, admission, scheduling, continuation, provider control plane, canonical model-family contracts, shared stateless/stateful lifecycles, scheduler-coupled media transactions and typed perception results, restart-safe generated-media output verticals, exported Zig package modules, an experimental C verifier, and a deterministic eight-profile retained-reference support registry/inspector with C, Python, and Rust queries | Common-plan binding and durable fresh-process resume for the text path, stable API policy, production fixtures, native Linux validation, product-specific build profiles, physical resource integration, and distribution |
