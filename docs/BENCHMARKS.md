...
`unary-http-test` covers four behavior scenarios: sibling liveness beside a
partial request; one-worker/one-pending accepted-FIFO dispatch with passive
...
shutdown, and final zero service/Bank ownership. A separate callback-order
inversion timestamp regression delivers the dispatch observer callback before
the enqueue observer callback while proving that both event timestamps remain
nonzero and ordered by lifecycle-mutex linearization.
...
| `python3 -m unittest bench.tests.test_lane4_token_txn_event_evidence bench.tests.test_token_txn_inspector` | Focused dependency-free TokenTxn replay and inspector gate: bounded canonical evidence and expectation inputs, a fixed journal receipt plus 64-wave/four-lane replay, exact caller-supplied root/head/resource/output matching, malformed and substitution rejection, metadata-only default rendering, and explicit complete token-ID disclosure. This is read-only Python contract evidence and intentionally compiles no Zig. It does not authenticate either input, select or issue the trusted expectation, grant runtime or disclosure authority, execute a model, or provide durability, native CPU/GPU, quality, or performance evidence. |
...
| `tools/zig-with-ephemeral-cache.sh build unary-server-process-test -Dmetal=false -Doptimize=ReleaseSafe -j2` | One dual-mode executable and one existing compile root retain real native POSIX child-process correctness for clean/restart lifecycle, receive drain/deadlines, admitted-work cancellation, reset handling, bounded response writes, full-request deadlines, fixed-worker/FIFO concurrency, queued deadlines, simultaneous drain, stale-owner cleanup, and exact application-rejection observations. Eight real `shutdown(.send)` controls add zero-byte/partial-head/partial-body receive classification, default completed-request response preservation, opt-in admitted-work/response-ready/response-writing cancellation, and final-send local completion with no post-send probe. Every profile retains exact counters, a healthy successor where applicable, joined cleanup, and zero final ownership. It adds no compile root or CI load profile and proves local lifecycle correctness only—not peer delivery, performance, concurrent model execution, GPU behavior, production-model quality, or native Windows/FreeBSD serving. |
| `zig build unary-http-test -Dmetal=false -Doptimize=ReleaseSafe -j2` | Focused serial lifecycle and Phase F1 native-loopback correctness over the production entry points. The serial regression proves the timeout-versus-local-completion boundary: a timeout claimed before completion publication remains the lifecycle winner while the physical transport outcome can stay `write_completed`; timely local completion cannot be relabelled by later retirement or cleanup. The concurrent scenarios retain bounded nonblocking accept/read revalidation, passive FIFO backpressure, queued timeout, simultaneous drain, exact event/snapshot conservation, callback-order evidence, joined shutdown, and zero ownership while model execution remains serialized. This is not real-process load/performance, GPU, native foreign-OS evidence, or peer-delivery acknowledgement; concurrent serving remains unsupported on Windows today. |
...
| `zig build unary-server-native-load-test -Dmetal=false -Doptimize=ReleaseSafe -j1` | Manual Phase F1 native CPU/loopback load capture on macOS or Linux. It reuses the existing `glacier-unary-server-process-test` artifact and compile root, runs one real child with 2 transport workers and 8 pending slots, and retains 8 warmup plus 64 measured requests across eight balanced flows. An independent Python verifier binds a fresh challenge, producer SHA-256 before and after execution, machine/boot identity, exact request-to-work/transport-owner and HTTP-response-handle correlation, output/terminal/completion roots, a 28-field terminal closure, and an embedded Native Workload Report V1. It reports completed-request arrival-to-FIFO accept/enqueue, FIFO queue, and HTTP first-positive-read distributions with an exact sample count of 64; terminal-response latency, throughput over the earliest measured arrival through latest measured settlement, and the all-completed outcome mix cover the measured cohort. Settlement is the later of client settlement and exact server transport/connection retirement. With 64 measured records, nearest-rank p99 is the maximum sample. The real target is opt-in and absent from automatic CI. `-Dunary-server-native-load-output=PATH` and `-Dunary-server-native-load-manifest-output=PATH` retain legacy non-authoritative compatibility files that are replaced independently, not as an atomic pair. `-Dunary-server-native-load-publication-output=PATH/report.glpub` instead emits one authoritative file binding the exact envelope and canonical context under a strict offline verifier and one-destination atomic replacement. Verify it without a producer or current-host probe with `python3 bench/native_unary_server_load.py --verify-publication PATH/report.glpub`. `.glpub` integrity does not authenticate capture origin or prove crash/power-loss durability. macOS can be environment-publication-eligible; Linux currently remains non-publishable by the CPU-attribution gate. This tiny serialized CPU fixture is not overload, large-model, first-token, fairness, physical-parallelism, GPU, power/energy/temperature, or cross-OS performance evidence. |
| `zig build unary-server-native-load-test -Dmetal=false -Doptimize=ReleaseSafe -j1 -Dunary-server-native-load-profile=retention-capacity-v1` | Manual Phase F1 retained-record-capacity capture through the same target, `glacier-unary-server-process-test` artifact, two-worker/eight-pending transport, environment gate, profile-specific outer sidecar, and embedded Native Workload Report V1. The fixed 72 records are 8 completed warmups plus a measured `32 completed / 32 HTTP 429 service_capacity` outcome mix, balanced as `4/4` in each of eight flows. Its completed-only accept/enqueue, FIFO queue, and HTTP first-positive-read distributions each retain exactly 32 samples; all 64 measured attempts remain in terminal/outcome accounting. For a rejection, the independent verifier checks the exact request/`service_capacity`/`same_request_after_backoff`/HTTP-429/response-byte-count semantic tuple, transport owner and lifecycle, and absence of created Service work or Bank ownership. The sidecar response-handle slot is a domain-separated producer-observed opaque raw-HTTP digest whose source bytes are not retained or reconstructable offline; its separately domain-separated `output_sha256` slot is the semantic root recomputed from that tuple. Completion binds both sidecar roots, while the embedded W6 output root stays zero because no model output exists. Service terminal-record capacity is exactly 40, and close requires zero connection, active service, Scheduler, and Bank ownership. This is retained-record capacity saturation, not transient or general overload, queued-timeout evidence, throughput superiority, production-model behavior, fairness, GPU behavior, or cross-OS evidence. |
| `zig build unary-server-native-load-test -Dmetal=false -Doptimize=ReleaseSafe -j1 -Dunary-server-native-load-profile=queued-receive-timeout-v1` | Manual Phase F1 deterministic closed-loop queued-pressure capture through the same target and `glacier-unary-server-process-test` artifact, with no new compile root and no live GitHub CI run. It completes 8 sequential warmups before 8 measured epochs. Each epoch holds 2 running requests, queues 6 peers until their exact 2-second receive timeout, then completes both running controls. Flow rotation retains measured `2 completed / 6 timed_out` in each of eight flows. Its completed-only accept/enqueue, FIFO queue, and HTTP first-positive-read distributions each retain exactly 16 samples; all 64 measured attempts remain in terminal/outcome accounting. Exact closure is enqueued `72`, dispatched/completed `24`, failed/queued-receive-timeout `48`, queue/running high-water `6/2`, pause/resume `8/8`, 24 completed Service records, event ordinal `184`, and zero live connection, Service, Scheduler, or Bank ownership. The verifier admits client arrival to timeout only in `2..4` seconds, enqueue to timeout at no more than 3 seconds, and timeout to peer-close/no-response transport settlement at no more than 1 second. Settlement requires zero response bytes and accepts a zero-length read or connection reset/abort; these are campaign-admissibility bounds, not general runtime latency promises. A timed-out W6 record retains only arrival, terminal, and settlement with not-applicable correctness and no Bank, work, dispatch, or output. Its outer record retains the client-observed canonical request root, an opaque raw-outbound-HTTP digest, exact enqueue/lease/`.queued_receive_timeout` evidence, and independently recomputable semantic, terminal, and completion roots. The queued connection emits only `.queued_receive_timeout`, never a separate `.retired` event; any `retired_*` sidecar fields are profile-defined aliases of that terminal event. It is never server-parsed; request-to-lease association is deterministic single-outstanding client-plan/transmit correlation, not server-parsed request attestation. This profile proves only its declared queue-pressure shape; it is not transient or general overload, generic queue latency, throughput superiority, fairness, physical parallelism, first-token latency, production-model, GPU, or native foreign-OS evidence. |
