...
| `tools/zig-with-ephemeral-cache.sh build unary-server-process-test -Dmetal=false -Doptimize=ReleaseSafe -j2` | R1k-b8 Phases A-D plus Phase E1 managed unary-server host-process gate. One dual-mode executable supervises and re-executes itself against a generated `32/64/1/256` ordinary package. Generation zero exits nonzero without `READY`; generation A proves model-list, completion/replay, malformed-peer survival, clean drain, restart identity, and zero ownership. Phase B uses exact `receiving_head` and `request_head_received` barriers to prove fenced receive-side drain cancellation with one failed connection and no service record. Phase C uses the same partial-head/body shapes to prove one bounded monotonic pre-admission receive deadline, distinct timeout evidence, same-child liveness, and no admission closure. Phase D adds synchronous barriers after exact active-work publication and before the serving thread's first drive operation. The cancellation-wins child invokes ordinary drain twice, validates a correlated `request_cancelled` error with retry policy `never`, and closes with one admitted-work cancellation at `request_admitted`. The completion-wins child first drives the one-token service request terminal while its HTTP lease remains published, invokes drain twice, and preserves the oracle-matched successful completion with zero work cancellations. Both close with one accepted and completed connection, zero failed connections, zero receive-drain and receive-timeout signals, zero active service requests, one terminal record, and zero Bank ownership. Phase E1 adds three deterministic post-admission controls. The real-peer reset child leaves one unread trailing byte before an abortive close and proves poll/`SO_ERROR` reset detection at the next between-quantum checkpoint, exact-handle work cancellation, open admission, same-child liveness, and separate peer-reset evidence. The `response_ready` drain child proves `cancelled_before_write` and one response cancellation before any write. The matching no-drain control retains the oracle response, records `write_completed`, and leaves every new cancellation counter zero. All three close with zero active service requests and Bank ownership. The barriers are deterministic rather than sleep or model-duration proxies. Frames remain bounded and expose no prompt or host path. Phase C is not a full-request timeout. Phase E1 does not detect orderly FIN abandonment, preempt an in-flight drive, interrupt a slow or already kernel-blocked response write, or prove peer delivery; `response_written` is local-flush evidence only. This gate adds no concurrent listener queue, process-wide overload policy, durable idempotency or crash recovery, load/performance behavior, streaming, authentication/TLS, GPU execution, production-model quality, or native Windows/FreeBSD serving proof; native reset, response-write, and deadline behavior on Windows and FreeBSD remains unproven. |
| `tools/zig-with-ephemeral-cache.sh build unary-server-process-compile -Dmetal=false -Doptimize=ReleaseSafe -j2` | Compile-only closure for the same managed controller/worker artifact. Phases B-E1 reuse this target and add no compile root. Complete affected verification selects it on retained targets only during explicit manual or milestone promotion; it launches no child and is portability evidence only, not native serving or lifecycle evidence for a foreign target. |
