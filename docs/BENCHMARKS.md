...
| `tools/zig-with-ephemeral-cache.sh build text-runtime-golden-path-test -Dmetal=false -Doptimize=ReleaseSafe -j2` | One compile-root package/raw-text gate: the Python controller verifies ordinary package admission, stages the direct one-token durable request, proves idempotent generation-one retry and foreign-input rejection before execution, continues in a fresh process, independently decodes both selected generations, compares the committed token with ordinary execution, checks terminal retry and one-shot behavior, and exercises bounded option rejection. This is clean process-continuation and local POSIX conformance evidence, not a crash, exhaustive storage/power-loss, GPU, production-quality, or performance claim. |
