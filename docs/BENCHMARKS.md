...
records and correctness-gated Metal controls.
...
the remaining W7b-b supervisor/in-flight/recovery-process and physical
storage/device disruption work remain staged. The sequence is defined in the
[Runtime Workload Lab](RUNTIME_WORKLOAD_LAB.md), with exact producers and
claim boundaries in the
...
| `tools/zig-with-ephemeral-cache.sh build native-metal-suite-test -Dmetal=true -Doptimize=ReleaseSafe -j2` | Serialized native macOS device suite: readiness → allocation ownership → production-native workload report → controlled disruption → cancellation storm → segmented clean-restart soak → post-segment process-kill recovery → fault/reconciliation → focused correctness and lifecycle no-event validation, with no overlap between those device gates; it combines their evidence boundaries and does not create a benchmark. Suite W6b report retention remains explicit through `-Dnative-metal-suite-report-output=PATH`; focused W7b reports and stores use their separate retention options. |
