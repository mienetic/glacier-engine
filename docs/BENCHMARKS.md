...
| `tools/zig-with-ephemeral-cache.sh build unary-server-process-test -Dmetal=false -Doptimize=ReleaseSafe -j2` | R1k-b8 Phases A-B managed unary-server host-process gate. One dual-mode executable supervises and re-executes itself against a generated `32/64/1/256` ordinary package. Generation zero exits nonzero without `READY`; generation A proves model-list, one completion, exact process-local replay, malformed-peer survival followed by a valid barrier request, exact `DRAINING`/`CLOSED` connection counts, zero active service requests, and zero Bank ownership after stdin `drain\n` plus EOF. Separate child generations hold a partial HTTP head open in `receiving_head` and a complete head with a declared but partial body open in `request_head_received`; drain closes completion admission, receive-shuts the generation/sequence/handle-fenced lease under the lifecycle lock, and the serving thread records one matching-phase drain signal plus exactly one accepted, zero completed, one failed, and zero active connection before its sole close. Both partial-receive children retain zero active service requests, zero terminal service records, and zero Bank ownership. Generation B loads the same package with a fresh process generation and idempotency key, proves the same model/binding/content/output identity, and closes cleanly. Frames are bounded and expose no prompt or host path. This is not a wall-clock timeout, post-admission accepted-work cancellation matrix, slow-response-write cancellation, concurrent/overload behavior, durable idempotency or crash recovery, load/performance behavior, streaming, authentication/TLS, GPU execution, production-model quality, or native Windows/FreeBSD serving proof; Windows pending-overlapped-receive shutdown remains unproven. |
| `tools/zig-with-ephemeral-cache.sh build unary-server-process-compile -Dmetal=false -Doptimize=ReleaseSafe -j2` | Compile-only closure for the same managed controller/worker artifact. Phase B reuses this target and adds no compile root. Complete affected verification selects it on retained targets; it launches no child and is portability evidence only, not native serving or lifecycle evidence for a foreign target. |
