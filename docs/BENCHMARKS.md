...
| `tools/zig-with-ephemeral-cache.sh build native-metal-allocation-test -Dmetal=true -Doptimize=ReleaseSafe -j2` | Hard native macOS Metal allocation-ownership gate: creates and directly inspects real Shared `MTLBuffer` resources, verifies exact logical ChildLease precharge/release, per-object device/length/`allocatedSize`, zero adapter-tracked objects plus zero independently registered shim resources after release, foreign/stale-token rejection, distinct adapter authorities, concurrent registry balance, and generation-fenced slot reuse; no residency, heap, allocation-gate dispatch, performance, broad-device, or multi-OS claim |
