...
manual native-load target now supplies one fixed all-completed CPU profile and
one fixed retained-record-capacity profile. Queued-timeout and broader
overload campaigns, production-model profiles, repeated captures, GPU
campaigns, and publication-eligible native Linux evidence remain pending.
