...
by a distinct production-linked 20-command W6 control.
...
the remaining W7b-b supervisor/recovery-process, active-kernel, adapter, and
physical storage/device disruption work remain staged. The sequence is defined
in the [Runtime Workload Lab](RUNTIME_WORKLOAD_LAB.md), with exact producers
and claim boundaries in the
...
| `tools/zig-with-ephemeral-cache.sh build native-metal-suite-compile -Dmetal=true -Doptimize=ReleaseSafe -j2` | Compile-only native Metal frontier: builds every test and executable consumed by the serialized suite, the device and host-tool compile profiles, the content-addressed Metal shader library, and the build-isolated fault-symbol check. The shader cache key is refreshed from the selected Xcode tools, SDK settings, and Metal standard-library contents. It opens no device and executes no GPU campaign, so success is build and static-isolation evidence only. `tools/verify.sh` runs this frontier before its separate `-j1` hardware phase with the same temporary caches and Metal output directory. |
| `tools/zig-with-ephemeral-cache.sh build native-metal-suite-test -Dmetal=true -Doptimize=ReleaseSafe -j1` | Serialized native macOS device suite: readiness → allocation ownership → production-native workload report → controlled disruption → cancellation storm → segmented clean-restart soak → post-segment process-kill recovery → controlled in-flight process kill → fault/reconciliation → focused correctness and lifecycle no-event validation, with no overlap between those device gates. Before each 60-second campaign, a bounded stabilizer requires two explicit nominal admission observations ten seconds apart within 180 seconds; timeout fails, while the campaign's own retained before/after checks still reject an in-run excursion. The suite combines their evidence boundaries and does not create a benchmark. Suite W6b report retention remains explicit through `-Dnative-metal-suite-report-output=PATH`; focused W7b reports and stores use their separate retention options. |
