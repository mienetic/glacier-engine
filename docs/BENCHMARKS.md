...
| `tools/zig-with-ephemeral-cache.sh build prepared-text-acknowledged-delivery-test -Dmetal=false -Doptimize=ReleaseSafe -j2` | Focused R1i contract gate for canonical result acknowledgements, idempotent sink replay/conflict/gap handling, immediate ACK-bound progress/terminal generations, retained predecessor loading, selector/identity rejection, and restored grant claim advancement. It is a deterministic local POSIX synthetic-fixture gate, not remote exactly-once, physical power-loss, GPU, Win32, production-model, or performance evidence. |
| `tools/zig-with-ephemeral-cache.sh build prepared-text-recovery-test -Dmetal=false -Doptimize=ReleaseSafe -j2` | Compile-once R1i campaign reusing one worker across 19 distinct target `SIGKILL` deaths: one post-step point, ten sink-publication points, one post-ACK point, and seven checkpoint-publication points. Independent decoding admits only previous or exact-successor roots and verifies three acknowledged suffix tokens plus the four-token terminal oracle. Scope begins after a clean generation-two source exit on the local POSIX synthetic fixture; source/initial-sink recovery, remote effects, physical power loss, GPU/Win32 behavior, and production execution remain outside the claim. |
