...
| `tools/zig-with-ephemeral-cache.sh build native-metal-fault-test -Dmetal=true -Doptimize=ReleaseSafe -j2` | Build-isolated native macOS Metal fault/reconciliation and retirement conformance: verifies the production shim exports no fault-control symbols and the non-installed fault shim exposes its bounded test ABI; two host threads race a context-local one-shot plan with exactly one winner; one real command physically completes successfully while physical facts and a test-published code-`11`-shaped `.error` overlay remain separate. The adapter exercises Phase A retention and exact active-pin binding, proves Bank-first settlement while the native record is live, exact-finalizes and stores a replay tombstone, then retries a deliberately rejected first confirmation from `settlement_pending` without a second Bank release or native finalization. The overlay is an explicitly synthetic authorization path and cannot satisfy the native-only production gate. A separate branch performs allocation retirement by releasing real buffer strong references plus logical LeaseTree charge under a synthetic test-only loss permit. This proves bounded native cleanup and retry ordering, not a physical driver, hardware, or device-loss fault, and is not performance or physical-reclaim evidence. |
...
behavior. The bounded retirement branch proves only quiesced reference and
