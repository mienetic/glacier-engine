...
| `tools/zig-with-ephemeral-cache.sh build prepared-text-acknowledged-delivery-test -Dmetal=false -Doptimize=ReleaseSafe -j2` | Focused R1j contract gate for canonical generation-one replay evidence, exact empty-sink admission, result acknowledgements, idempotent sink replay/conflict/gap handling, immediate ACK-bound progress/terminal generations, retained predecessor loading, selector/identity rejection, and restored grant claim advancement. It is a deterministic local POSIX synthetic-fixture gate, not remote exactly-once, hostile-writer or physical power-loss, GPU, Win32, production-model, or performance evidence. |
| `tools/zig-with-ephemeral-cache.sh build prepared-text-result-inspector-test -Dmetal=false -Doptimize=ReleaseSafe -j2` | Focused R1k-b3 committed-output inspection gate for aligned and sink-exactly-one-ahead reconciliation, terminal aligned state with a nonempty acknowledgement prefix, exact `utf8-byte-v1` token bytes bounded to `0...255`, metadata-only default output, explicit exact-byte disclosure, nullable strict UTF-8 text, shared Zig/Python roots, failure-atomic caller output, one real `after_sink_before_selector` process death, and unchanged bounded directory manifests around both inspector calls. This is deterministic read-only evidence over a cooperative local POSIX fixture. It creates no lease or authority and proves no hostile-writer defense, authentication, payload privacy, physical power-loss persistence, remote exactly-once delivery, GPU or Win32 behavior, native multi-OS support, production-model execution, ordinary serving integration, or performance. |
| `tools/zig-with-ephemeral-cache.sh build prepared-text-recovery-test -Dmetal=false -Doptimize=ReleaseSafe -j2` | Compile-once R1j campaign reusing one worker across 49 distinct `SIGKILL` deaths: seven generation-one bootstrap, 23 source-transition, and 19 target-transition boundaries. Each victim emits a gated ready frame and self-raises the real signal. Independent decoding admits only the boundary's declared absent, exact-predecessor, or exact-successor checkpoint/sink state, then verifies three acknowledged suffix tokens plus the four-token terminal oracle. Scope is the local pre-tokenized POSIX synthetic fixture; source replay after external effects, remote delivery, hostile writers, physical power loss, GPU/Win32 behavior, and production execution remain outside the claim. |
