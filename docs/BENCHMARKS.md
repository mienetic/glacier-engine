...
| `zig build dense-tensor-reranker-test -Dmetal=false` | Download-free generic reranking with canonical batch identity, exact integer scores, deterministic ties, rooted ranked items, direct/scheduled publication, cancellation, independent Python recomputation, and final zero ownership; no quality or performance claim |
| `zig build dense-tensor-embedding-test -Dmetal=false` | Download-free dense projection with canonical batch identity, checked integer accumulation, exact Q30 L2 squared-threshold normalization, direct/scheduled publication, cancellation, independent Python big-integer recomputation, and final zero ownership; no quality or performance claim |
...
| `tools/zig-with-ephemeral-cache.sh build text-runtime-golden-path-test -Dmetal=false -Doptimize=ReleaseSafe -j2` | One compile-root package/raw-text gate using the staged production `bin/glacier`: the Python controller runs CLI children from an empty working directory with isolated ambient state, requires the installed binary namespace to remain byte-identical, derives deterministic in-vocabulary process-local early-EOS, EOS-at-limit, and EOS-miss cases, independently recomputes variable-terminal and completed-early roots, proves unused-quanta closure and durable-EOS rejection before mutation; retains the direct `N=1` bootstrap/retry/fresh-process/terminal checks; exercises acknowledged `N=4` bootstrap retry, changed-count and foreign-input no-mutation rejection, fresh-process continuation, complete independent checkpoint/sink lineage decoding, ordinary-output equality, and byte-identical terminal retry; and covers one-shot `N=2` and `N=64` endpoints so capacities `1`, `3`, and `63` are verified. It adds no executable compile root. This is installed-shape, same-host process-local terminal and clean process-continuation/POSIX conformance evidence, not a crash, exhaustive storage/power-loss, native multi-OS, GPU, production-quality, or performance claim. |
| `tools/zig-with-ephemeral-cache.sh build unary-server-process-test -Dmetal=false -Doptimize=ReleaseSafe -j2` | R1k-b8 Phases A-C managed unary-server host-process gate. One dual-mode executable supervises and re-executes itself against a generated `32/64/1/256` ordinary package. Generation zero exits nonzero without `READY`; generation A proves model-list, one completion, exact process-local replay, malformed-peer survival followed by a valid barrier request, exact `DRAINING`/`CLOSED` connection counts, zero active service requests, and zero Bank ownership after stdin `drain\n` plus EOF. Phase B child generations hold a partial HTTP head open in `receiving_head` and a complete head with a declared but partial body open in `request_head_received`; each enables a long receive deadline, then drain closes completion admission, receive-shuts the generation/sequence/handle-fenced lease under the lifecycle lock, and proves that shutdown wakes the deadline reader with one matching-phase drain signal, no timeout signal, exactly one accepted, zero completed, one failed, and zero active connection before the serving thread's sole close. Phase C adds a managed-only monotonic receive deadline, disabled by zero and bounded from 1 millisecond through 60 seconds. Two further generations use one second to expire the same partial-head/body shapes without drain, publish exact `TIMED_OUT` phase evidence with drain count zero, send no response to the interrupted peer, leave lifecycle ready and completion admission open, serve a valid model-list request in the same child, then close with two accepted, one completed, one failed, one timeout, zero active service requests, zero terminal service records, and zero Bank ownership. One timer starts immediately after accept, never resets after the head, and is joined before the serving thread retires and solely closes the lease. The partial-head case also sends three incremental bytes inside that second and retains an upper timing bound, proving peer progress does not convert the deadline into an inactivity timeout. Generation B loads the same package with a fresh process generation and idempotency key, proves the same model/binding/content/output identity, and closes cleanly. Frames are bounded and expose no prompt or host path. Phase C is not a full-request timeout, post-admission accepted-work cancellation, or slow-response-write cancellation. This gate adds no concurrent listener queue, process-wide overload policy, durable idempotency or crash recovery, load/performance behavior, streaming, authentication/TLS, GPU execution, production-model quality, or native Windows/FreeBSD serving proof; native deadline behavior on Windows and FreeBSD remains unproven. |
| `tools/zig-with-ephemeral-cache.sh build unary-server-process-compile -Dmetal=false -Doptimize=ReleaseSafe -j2` | Compile-only closure for the same managed controller/worker artifact. Phases B-C reuse this target and add no compile root. Complete affected verification selects it on retained targets; it launches no child and is portability evidence only, not native serving or lifecycle evidence for a foreign target. |
