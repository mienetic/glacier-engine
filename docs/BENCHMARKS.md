...
native-loopback correctness are retained. The passing ReleaseSafe
...
That is same-process native-loopback evidence. The current
`unary-server-process-test` stops at Phase E2b and does not execute
`serveManagedConcurrentListenerV1` or the Phase F1
fixed-worker/FIFO/shared-watchdog path. It must not be cited as Phase F1
real-process evidence. Native load/performance, GPU behavior, and native
foreign-OS behavior remain separate pending campaigns.
