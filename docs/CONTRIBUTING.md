...
- Require a complete caller-supplied expectation when an inspector cannot
  derive trusted identity independently. State that matching verifies
  integrity and composition relative to that expectation, not its
...
| Portable workload report/campaign codec, verifier, or focused test | `affected-fast` runs changed-file Python syntax where applicable plus the four host test roots in one Zig invocation; it does not select native compile-only or foreign cross-compile roots. Complete `affected` adds those compile and fixed foreign cross-compile closures. Neither tier runs a hard native campaign unless another changed path selects one |
...
| Native unary load `.glpub` publication codec, workflow, or focused test | Run `python3 -m unittest bench.tests.test_native_unary_server_load_publication`; this standard-library-only structural gate adds no Zig compile. Require fixed magic/ABI and zero flags, canonical compact object JSON, exact bounded lengths with no truncation or trailing bytes, raw manifest/envelope digests, the domain-separated publication identity and seal, strict read-time identity/stability checks, and verified same-directory one-file atomic replacement. The manual target emits the bundle only when `-Dunary-server-native-load-publication-output=PATH/report.glpub` is supplied; verify it offline with `python3 bench/native_unary_server_load.py --verify-publication PATH/report.glpub`. Its legacy envelope/manifest outputs remain non-authoritative and independently atomic. The bundle provides integrity and exact composition, not authentication, trusted provenance, crash recovery, hostile-writer defense, or physical power-loss durability |
| Python TokenTxn replay verifier, inspector, or their focused regressions | `affected-fast` and `affected` run changed-file Python syntax plus `python3 -m unittest bench.tests.test_lane4_token_txn_event_evidence bench.tests.test_token_txn_inspector`; the focused route selects no Zig build, foreign target, native runtime, model, or GPU gate |
...
python3 -m unittest bench.tests.test_native_unary_server_load_publication
...
zig build unary-server-native-load-test \
  -Dmetal=false -Doptimize=ReleaseSafe -j1 \
  -Dunary-server-native-load-output=PATH/report.bin \
  -Dunary-server-native-load-manifest-output=PATH/manifest.json
